create or replace PACKAGE BODY         "ENEL_ACTUALIZA_INFORMES_CEBP" AS
/* *****************************************************************************
    NAME:       ACTUALIZA_INFORMES_CEBP
    PURPOSE:

    REVISIONS:
    Ver              Date            Author               Description
    ---------          ----------      ---------------      -----------------------------------                                            
    1.0                15/06/2020        MPR                    Creacion de procedimientos para nueva processingUnit - CE_BP
    2.0             07/02/2022      DCR                 Informe Agregado no coherente con cuadre de liquidación, Canal BP
    3.0             29/05/2024      APM                 Se añade nuevo campo Decomisión a la tabla ENEL_CUADRELIQ_FINAL_CES    
    4.0             25/06/2024      APM                 Actualización CAMPO5 se rellena del DEPOSIT.GD1.
    ...
    9.0             25/02/2026      APM                 En el insert de la tabla ENEL_DET_INCENTIVOS_CES se comenta lógica trimestral nn,
                                                        y se añade filtro de que no coja comisiones de importe base.
    10.0            26/02/2026      APM                 Cambios en los join de la tabla ENEL_DET_INCENTIVOS_CES referente al periodo.                                                     
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CEBP.', v_contador_debug);
       w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CEBP_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CEBP_2';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CEBP_2.', v_contador_debug); 
end;
...
begin
    w_debug('Cargando tabla ENEL_TXN_TEMP_CEBP. Periodo:'|| iperiod ,  v_contador_debug);
    v_periodstartdate :=  f_fecha_inicio(iperiodseq);
    v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
    
    INSERT INTO ENELEXT.ENEL_TXN_TEMP_CEBP( TENANTID, PERIODSEQ, PERIODO, SALESORDERSEQ, SALESTRANSACTIONSEQ, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTYPEID, COMPENSATIONDATE, ACCOUNTINGDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_TXN_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    BEGIN
        SYS.DBMS_STATS.GATHER_TABLE_STATS (
        OwnName => 'ENELEXT'
        ,TabName => 'ENEL_TXN_TEMP_CEBP'
        ,Estimate_Percent => 1
    ...
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_TXN_TEMP_CEBP.',v_contador_debug);
    
    
     w_debug('Cargando tabla ENEL_TXN_TEMP_CEBP_2. Periodo:'|| iperiod ,  v_contador_debug);
    v_periodstartdate :=  f_fecha_inicio(iperiodseq);
    v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
    
    INSERT INTO ENELEXT.ENEL_TXN_TEMP_CEBP_2( TENANTID, PERIODSEQ, PERIODO, SALESORDERSEQ, SALESTRANSACTIONSEQ, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTYPEID, COMPENSATIONDATE, ACCOUNTINGDATE, 
                                        PRODUCTID, GENERICATTRIBUTE1, GENERICATTRIBUTE2, GENERICATTRIBUTE3, GENERICATTRIBUTE4, GENERICATTRIBUTE5, GENERICATTRIBUTE6, GENERICATTRIBUTE7, 
                                        GENERICATTRIBUTE8, GENERICATTRIBUTE9, GENERICATTRIBUTE10, GENERICATTRIBUTE11, GENERICATTRIBUTE12, GENERICATTRIBUTE13, GENERICATTRIBUTE14, 
                                        GENERICATTRIBUTE15, GENERICATTRIBUTE16, GENERICATTRIBUTE17, GENERICATTRIBUTE18,GENERICATTRIBUTE19, GENERICATTRIBUTE20, 
                                        GENERICATTRIBUTE21, GENERICNUMBER1, GENERICNUMBER2, GENERICNUMBER3, GENERICDATE3, GENERICDATE4, GENERICDATE5, GENERICBOOLEAN2, 
...
/*BOM APM 26.02.2026 New Code*/
        INNER JOIN CS_PERIOD PER 
            on TXN.compensationdate >= per.startdate 
            and TXN.compensationdate < per.enddate 
            and per.periodtypeseq= 2814749767106561 and 
            per.removedate=TO_DATE ('01/01/2200','DD/MM/YYYY') 
/*EOM APM 26.02.2026*/            
    ...
    w_debug('Fin Carga de la tabla ENEL_TXN_TEMP_CEBP_2: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Borrado de la tabla ENEL_MEDIDAS_TEMP_CEBP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_MEDIDAS_TEMP_CEBP WHERE  ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_MEDIDAS_TEMP_CEBP.', v_contador_debug);

    w_debug('Insertando datos en tabla ENEL_MEDIDAS_TEMP_CEBP.' ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_MEDIDAS_TEMP_CEBP (TENANTID,PERIODSEQ, PERIODO, NAME, VALUE, MEASUREMENTSEQ, PAYEESEQ, POSITIONSEQ, PIPELINERUNSEQ, PLANSEQ, RULESEQ)
...
/*      
  ...
  */              
    ...
    w_debug('Fin Carga de la tabla ENEL_MEDIDAS_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_MEDIDAS_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_MEDIDAS_TEMP_CEBP',v_contador_debug);
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CEBP.', v_contador_debug);
      w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CEBP_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEBP_2';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CEBP_2.', v_contador_debug);

    w_debug('Cargando tabla ENEL_CREDIT_TEMP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CEBP( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
    ...
    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
...
v_periodstartdate :=  f_fecha_inicio(iperiodseq);
    v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
    w_debug('Cargando tabla ENEL_CREDIT_TEMP_CEBP_2. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CEBP_2( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
                                            COMMENTS, CREDITTYPEID, CREDITTYPEDESCRIPT, VALUE, PREADJUSTEDVALUE, GENERICATTRIBUTE1, GENERICATTRIBUTE2, GENERICATTRIBUTE3, GENERICATTRIBUTE4, 
                                            GENERICATTRIBUTE5, GENERICATTRIBUTE6, GENERICATTRIBUTE7, GENERICATTRIBUTE8, GENERICATTRIBUTE9, GENERICATTRIBUTE10, GENERICATTRIBUTE11, 
                                            GENERICATTRIBUTE12, GENERICATTRIBUTE13, GENERICATTRIBUTE14,GENERICATTRIBUTE15, GENERICBOOLEAN1, GENERICBOOLEAN2, GENERICDATE1, 
                                            GENERICNUMBER1, GENERICNUMBER2, GENERICNUMBER3, GENERICNUMBER4, BU_NAME, GENERICNUMBER6 )
...
/*BOM APM 26.02.2026 New Code*/
        INNER JOIN cs_period per
            on credit.compensationdate >= per.startdate 
            and credit.compensationdate < per.enddate 
...
/*EOM APM 26.02.2026*/
    ...
    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CEBP_2: '|| to_char(filas) || ' filas.', v_contador_debug);


    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_CREDIT_TEMP_CEBP.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_COMMISSION_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_COMMISSION_TEMP_CEBP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_COMMISSION_TEMP_CEBP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_COMMISSION_TEMP_CEBP( TENANTID, PERIODSEQ, PERIODO, COMMISSIONSEQ, CREDITSEQ, INCENTIVESEQ, ENTRYNUMBER, PIPELINERUNSEQ, PAYEESEQ, POSITIONSEQ, ORIGINTYPEID, PIPELINERUNDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_COMMISSION_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMMISSION_TEMP_CEBP.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_INCEN_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_INCEN_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_INCEN_TEMP_CEBP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_INCEN_TEMP_CEBP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_INCEN_TEMP_CEBP(  TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, INCENTIVESEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE,
    ...
    w_debug('Fin Carga de la tabla ENEL_INCEN_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_INCEN_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_INCEN_TEMP_CEBP.',v_contador_debug);
end;         
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_DEPOSIT_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_DEPOSIT_TEMP_CEBP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_DEPOSIT_TEMP_CEBP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_DEPOSIT_TEMP_CEBP( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, DEPOSITSEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE, 
                                                PREADJUSTEDVALUE, EARNINGCODEID, EARNINGGROUPID, COMMENTS, GENERICATTRIBUTE1, GENERICATTRIBUTE2, tipo_pago_ga5, 
                                                processingunitseq, wbe, BUSINESSUNITMAP, LINEA_NEGOCIO, TIPO, BU_NAME, GENERICNUMBER1,
                                                GENERICDATE1,GENERICATTRIBUTE9) --APM 25.06.2024
    SELECT
        depo.TENANTID,
        depo.PERIODSEQ,
        iperiod PERIODO,
    ...
    w_debug('Fin Carga de la tabla ENEL_DEPOSIT_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_DEPOSIT_TEMP_CEBP.',v_contador_debug);   
end;         
...
begin
    w_debug('Inicio Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEBP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_ORDER_IMPU_TEMP_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP( TENANTID,PERIODSEQ,IDPROVEEDOR,DESCRIPCION,DESCRIPCION_CORTA, FICHERO,CECO,
    ...
    w_debug('Fin Carga de la tabla ENEL_ORDER_IMPU_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP.',v_contador_debug); 
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CEBP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_EQUIPAMIENTO_TEMP_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CEBP( TENANTID,PERIODSEQ,EQUIPAMIENTO, IDMARCA, MARCA, IDMODELO, MODELO, IDTIPO,
    ...
    w_debug('Fin Carga de la tabla ENEL_EQUIPAMIENTO_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEBP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CEBP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEBP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Origen de ENEL_E4E_NEGATIVOS_TEMP_CEBP : CS_DEPOSIT.', v_contador_debug);
    ...
    INSERT INTO ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CEBP( PERIODSEQ,PERIODO, DEPOSITSEQ,POSITIONSEQ,PAYEESEQ,VALUE,PDS,
    ...
    FROM ENEL_DEPOSIT_TEMP_CEBP DEPO
        ...
        LEFT JOIN ENEL_E4E_CONTRATOS_TEMP_CEBP TMP_CONTRA
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CEBP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_E4E_CONTRATOS_TEMP_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEBP(TENANTID,PERIODSEQ,ID,PDS,ACTIVIDAD_DETALLADA,ACTIVIDAD, CIF,COD_CONTRATO,POS_DOC,TEXTO_BREVE,CODIGO_SERVICIO,
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_CONTRATOS_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEBP.',v_contador_debug);   
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_PDS_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_PDS_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_PDS_TEMP_CEBP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_PDS_TEMP_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_PDS_TEMP_CEBP( PERIODSEQ, RULEELEMENTOWNERSEQ, PAYEESEQ, PAYEEID, PDS, NOMBRE_FISCAL, CIF, NOMBRE_CUENTA, CALLE, COD_POSTAL, 
                                          ...
                                          BASE_COMISION, FECHA_INI_VIGENCIA,TERMINATIONDATE,TITLE_NAME,CANAL_CALCULOS,TIPO_POSICION, NOMBRE_FISCAL_CORTO, PLAN_NAME     )
    SELECT 
        PER.PERIODSEQ, 
    ...
    FROM CS_PERIOD per
        ...
        --MPR se incluye el plan para aplicar los filtros en cuadre de liq.
        LEFT JOIN CS_PLANASSIGNABLE PLN
            ON POS.RULEELEMENTOWNERSEQ = PLN.RULEELEMENTOWNERSEQ
            AND PLN.REMOVEDATE = v_eot
            AND PLN.TENANTID = itenantId

        LEFT JOIN CS_PLAN PN 
            ON PLN.PLANSEQ = PN.RULEELEMENTOWNERSEQ
            AND PN.REMOVEDATE = v_eot
            AND PN.TENANTID = itenantId
        --Se incluye condición con las EFFECTIVESTARTDATE y EFFECTIVEENDDATE del Plan para evitar duplicados
            AND PN.EFFECTIVESTARTDATE <= PER.ENDDATE - 1
            AND PN.EFFECTIVEENDDATE >= PER.ENDDATE - 1
            AND PN.MODELSEQ = 0
    ...
    w_debug('Fin Carga de la tabla ENEL_PDS_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PDS_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PDS_TEMP_CEBP.',v_contador_debug);
end;
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CEBP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEBP';
    w_debug('Fin Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CEBP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_E4E_DEPOSIT_TEMP_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    -- Si se ejecuta en una FASE que no es REWARD Utilizamos la tabla de PAGOS
    w_debug('Origen de ENEL_E4E_DEPOSIT_TEMP_CEBP.', v_contador_debug);

    INSERT INTO ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEBP( PERIODSEQ, PERIODO, POSITIONSEQ,PAYEESEQ,VALUE,PDS,PAR_PROVEEDOR,TIPO_IMPOSITIVO,COD_CONTRATO,POS_DOC,
                                                ...
                                                SUBPOSICION, BUSINESSUNIT, LINEA_NEGOCIO, TIPO, CAMPANIA, NOM_SOLICITANTE, POS_ORDEN, LINEA_SERVICIO,
                                                GENERICDATE1,GENERICATTRIBUTE9) --APM 25.06.2024
    Select
        DEPO.PERIODSEQ,
        iperiod,
    ...
    FROM  ENELEXT.enel_deposit_temp_CEBP DEPO                 
        INNER JOIN ENELEXT.ENEL_PDS_TEMP_CEBP TMP_PDS 
            ON depo.payeeseq=TMP_PDS.payeeseq 
            and depo.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
            and depo.periodseq=TMP_PDS.periodseq

        INNER JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP TMP_PROV 
            ON TMP_PROV.IDPROVEEDOR=depo.earninggroupid  
            AND depo.periodseq=TMP_PROV.periodseq

        LEFT JOIN ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEBP TMP_CONTRA
            ON TMP_CONTRA.periodseq=depo.periodseq
            AND TMP_CONTRA.PDS = TMP_PDS.PDS
            AND TMP_CONTRA.ACTIVIDAD_DETALLADA=TMP_PROV.DETALLE_ACTIVIDAD
            AND TMP_CONTRA.periodseq=TMP_PROV.periodseq
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_DEPOSIT_TEMP_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEBP.',v_contador_debug);
end;
...
AS
    contador integer;  
begin
    w_debug('Inicio Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CEBP para fichero ' || iFichero ,  v_contador_debug);
    contador := 1;
    -- Registro de CABECERA 1 : lista de campos
    INSERT INTO ENEL_E4E_FINAL_CEBP (PERIODO, ORDEN, CAMPO1, CAMPO2, CAMPO3, CAMPO4, CAMPO5, CAMPO6, CAMPO7, CAMPO8, CAMPO9, CAMPO10, 
    ...
    'Org.Compras','Gr.Compras','Riesgo','Contract Manager','Sit.Trabajo','Nota Cab.','Interlocutor DP','Interlocutor EF','Descripcion Breve','Contract Supervisor','','','',iFichero, 'BUSINESSUNIT');--dms 28.06.2024 interl dp hasta contract supervisor
    ...
    COMMIT;
    w_debug('Fin Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CEBP para fichero ' || iFichero ,  v_contador_debug);
...
AS
    ...
    v_aux VARCHAR(10);
begin
    w_debug('Inicio Borrado de la tabla ENEL_E4E_FINAL_CEBP.', v_contador_debug);
   
    --v_codFichero :='E4E1';  -- v2.0 se asigna el valor dinamicamente en funcion de la actividad del proveedor
        
    -- EXECUTE IMMEDIATE 'DELETE ENELEXT.ENEL_E4E_FINAL_CEBP WHERE ....';
    BEGIN
        LOOP
            DELETE FROM ENEL_E4E_FINAL_CEBP WHERE PERIODO = iperiod AND FICHERO like 'E%1' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_E4E_FINAL_CEBP.', v_contador_debug);
    ...
    BEGIN
        LOOP
            DELETE FROM ENEL_AGR_FAC_E4E_FINAL_CES WHERE PERIODO = iperiod AND CANAL = 'BP' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;    
    ...
    w_debug('Cargando tabla ENEL_E4E_FINAL_CEBP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    w_debug('Insertando Registros de datos en tabla ENEL_E4E_FINAL_CEBP. Fichero ' || v_codFichero ,  v_contador_debug);
    ...
    DECLARE
        CURSOR C_TMPDEPOSITOS IS
            SELECT 
                ...
                POS_ORDEN,
                LINEA_SERVICIO,
                GENERICDATE1, --APM 25.06.2024
                GENERICATTRIBUTE9 --DMS 28.06.2024
            FROM ENEL_E4E_DEPOSIT_TEMP_CEBP
        ...
        LOOP        
            ...
            IF REGDEPOSITO.TIPO_PAGO = 'SERVICIO' THEN
                ...
                VALUES ( iperiod,
                        ...
                        -- REQ-CM-003: Informe de Balance para Clientes Empresa - inicio - 05.11.2021
                        INSERT INTO ENELEXT.ENEL_AGR_FAC_E4E_FINAL_CES  (PERIODO, PDS, IMPORTE_NUM, DET_ORDEN, CANAL, 
                        LINEA_NEGOCIO)
                        VALUES ( iperiod,
                        --REGDEPOSITO.COD_CONTRATO,
                        --SUBSTR(v_cod_prefactura,0,14),
                        --v_referencia,
                        v_cod_prefactura,
                        REGDEPOSITO.VALUE,
                        v_referencia,
                        'BP',
                        --REGDEPOSITO.LINEA_NEGOCIO
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CEBP:  E4E1 '|| to_char(contadorE4E) || ' -- ECS1 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
end;
...
begin
    ...
    BEGIN
        LOOP
            DELETE FROM ENEL_E4E_FINAL_CEBP WHERE PERIODO = iperiod AND FICHERO like 'E%2'  AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CEBP:  E4E2 '|| to_char(contadorE4E) || ' -- ECS2 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
...
begin
    ...
    FROM ENEL_E4E_NEGATIVOS_TEMP_CEBP e4edt 
...
begin
    w_debug('Inicio Borrado de la tabla ENEL_LIQUIDACION_CEBP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_LIQUIDACION_CEBP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_LIQUIDACION_CEBP.', v_contador_debug);
    ...
    w_debug('Insertando CREDITOS de datos en tabla ENEL_LIQUIDACION_CEBP.' ,  v_contador_debug);
    -- v2.0 Se cambia la tabla de origen CS_CREDIT  a la temporal ENEL_CREDIT_TEMP
    INSERT INTO ENELEXT.ENEL_LIQUIDACION_CEBP ( PERIODO, PROVEEDOR, ANO_LIQUIDACION, MES_LIQUIDACION, CODIGO_AGENTE_INTERNO, CONCEPTO, CANTIDAD, 
    ...
    FROM ENEL_CREDIT_TEMP_CEBP credtmp
        INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS 
            ON credtmp.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
        LEFT JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CEBP TMP_PROV
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_LIQUIDACION_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando INCENTIVOS de datos en tabla ENEL_LIQUIDACION_CEBP.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CEBP INCENTMP    
        INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_LIQUIDACION_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_LIQUIDACION_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_LIQUIDACION_CEBP.',v_contador_debug);
end;
...
begin
    ...
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_DET_INCENTIVOS_CES WHERE (PERIODO_REAL = iperiod or (periodo_real is null and periodo=iperiod)) AND PROCESSINGUNITSEQ = iprocessingUnitSeq AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    SELECT 
        ...
        TMP_PDS.NOMBRE_FISCAL,
        ECT.GENERICATTRIBUTE2 AS LINEA_NEGOCIO, -- GA2
        ...
        ETT.GENERICATTRIBUTE21 as CONDICION_MULTIPUNTO,-- GA21
    ...
    FROM ENEL_TXN_TEMP_CEBP ETT
        LEFT JOIN ENEL_CREDIT_TEMP_CEBP ECT
            ON ECT.SALESTRANSACTIONSEQ = ETT.SALESTRANSACTIONSEQ
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CEBP TEMP_PROV
            ON TEMP_PROV.IDPROVEEDOR=ECT.GENERICATTRIBUTE4
        LEFT JOIN ENEL_PDS_TEMP_CEBP TMP_PDS 
            ON ECT.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ 
            
    WHERE ((ECT.VALUE IS NOT NULL AND TMP_PDS.TIPO_PRESTADOR = 'SI')    -- sólo afecta a canal BP (oct. 2020)
            OR ECT.VALUE IS NULL)
    ;
...
/*EOM APM 26.02.2026 New Code*/
    ...
    INSERT INTO ENELEXT.ENEL_DET_INCENTIVOS_CES (PERIODO, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTTYPEID, IMPORTE, UNIDAD, VALOR_1, UNIDAD_1, PRODUCTO_SCAWEB, 
                                                    ...
                                                    ESTADO_CTRLCALIDAD, Nom_Credito, PRODUCTO, CAMPANIA, TARIFA, CONSUMO, DESCUENTO, POTENCIA, PVP,
                                                    NOMBRE_FISCAL, LINEA_NEGOCIO, FECHA_INSERCION, FECHA_FIRMA, PROCESSINGUNITSEQ, CANAL, OFERTA, AGENTE, COMENTARIOS, ANIO, 
                                                    CUPS, CIF, NOMBRE_CLIENTE, SEGMENTO, PROVINCIA, TERRITORIO, CONSUMO_TXN, COMPONENTE, PAYEESEQ, POSITIONSEQ,NOMBRE_INCENTIVO,CONTRATO,PERIODO_REAL)
    SELECT 
       /*BOM APM 25.02.2026 Old Code*/
        /*CASE 
       WHEN ince.name = 'C - CE - BP - Electricidad - Incentivo Objetivos (%) Trimestral Renovacion NN 0' 
             OR ince.name = 'C - CE - BP - Electricidad - Incentivo Objetivos (%) Trimestral Renovacion NN -1'
             OR ince.name = 'C - CE - BP - Electricidad - Incentivo Objetivos (%) Trimestral Renovacion NN -2'
        THEN 
            CASE 
                WHEN ETT.PERIODO LIKE 'Marzo%' OR ETT.PERIODO LIKE 'Abril%' OR ETT.PERIODO LIKE 'Mayo%'
                THEN 'Mayo ' || v_anio
                WHEN ETT.PERIODO LIKE 'Junio%' OR ETT.PERIODO LIKE 'Julio%' OR ETT.PERIODO LIKE 'Agosto%'
                THEN 'Agosto ' || v_anio
                WHEN ETT.PERIODO LIKE 'Septiembre%' OR ETT.PERIODO LIKE 'Octubre%' OR ETT.PERIODO LIKE 'Noviembre%'
                THEN 'Noviembre ' || v_anio
                WHEN ETT.PERIODO LIKE 'Diciembre%' 
                THEN 'Febrero ' || TO_CHAR(TO_NUMBER(v_anio) + 1)
                WHEN ETT.PERIODO LIKE 'Enero%' or ETT.PERIODO LIKE 'Febrero%'
                THEN 'Febrero ' || v_anio
            END
            ELSE iperiod
        END AS PERIODO,*/
        --New Code
        ETT.PERIODO as PERIODO,
        /*EOM APM 25.02.2026*/
        ETT.ORDERID,
        ETT.LINENUMBER,
        ETT.SUBLINENUMBER,
        ...
        TMP_PDS.PAYEESEQ,
        TMP_PDS.RULEELEMENTOWNERSEQ,  -- POSITIONSEQ
        INCE.NAME AS NOMBRE_INCENTIVO,
        ETT.PONUMBER AS CONTRATO,
        ETT.PERIODO
        
        
    FROM ENEL_TXN_TEMP_CEBP_2 ETT
        LEFT JOIN ENEL_CREDIT_TEMP_CEBP_2 ECT
            ON ECT.SALESTRANSACTIONSEQ = ETT.SALESTRANSACTIONSEQ
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CEBP TEMP_PROV
            ON TEMP_PROV.IDPROVEEDOR=ECT.GENERICATTRIBUTE4
            and TEMP_PROV.periodseq = ect.periodseq --APM 26.02.2026
        INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS 
            ON ECT.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ 
            and TMP_PDS.periodseq =ect.periodseq --APM 26.02.2026
        INNER JOIN cs_commission commi
            on commi.CREDITSEQ = ECT.creditseq
            and commi.payeeseq = ECT.payeeseq
            AND COMMI.PERIODSEQ=ECT.PERIODSEQ --APM 26.02.2026
    ...
    WHERE TMP_PDS.PLAN_NAME like 'Clientes Empresa - Business Partners (BP)'
    /*BOM APM 25.02.2026 New Code*/
        and (ince.name not like 'C - CE - BP - Electricidad - Importe Base%'
        and ince.name not like 'C - CE - BP - Gas - Importe Base%')
    /*EOM APM 25.02.2026*/
    ;
...
begin
    ...
    from ENELEXT.ENEL_INCEN_TEMP_CEBP edt
        INNER JOIN ENEL_PDS_TEMP_CEBP CSP
...
--        and value > 0  7feb22 Se solicita que aparezcan negativos en Cuadre Liq/Total Precio Base Fijo
        and CSP.TIPO_PRESTADOR = 'SI' -- sólo afecta a canal BP (oct. 2020)
        ;
    ...
    FROM (  SELECT
                EDT.periodo,
                EDT.BU_NAME || v_mesanio || CSP.PDS || SUBSTR(depo.linea_negocio,0,1) || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) as cod_prefactura,
                EDT.VALUE,
                CSP.PDS || 
                  CASE
                    ...
                    --ELSE SUBSTR(depo.linea_negocio,0,1) || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    ELSE 'ECPF'
                  END
                  /*CASE
        ...
        from ENELEXT.ENEL_INCEN_TEMP_CEBP edt
            INNER JOIN ENEL_PDS_TEMP_CEBP CSP
                ON EDT.POSITIONSEQ=CSP.RULEELEMENTOWNERSEQ
            INNER JOIN enelext.ENEL_DEPOSIT_TEMP_CEBP DEPO
                ON DEPO.payeeseq=csp.payeeseq 
                AND DEPO.POSITIONSEQ=csp.RULEELEMENTOWNERSEQ
                AND DEPO.periodseq=csp.periodseq
        where 
            (edt.name  like 'I - CE - % - Precio Base Fijo%')
            and edt.value > 0
            and CSP.TIPO_PRESTADOR = 'SI' -- sólo afecta a canal BP (oct. 2020)
    )
...
begin
    ...
    FROM ENELEXT.ENEL_INCEN_TEMP_CEBP CSI
           INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS
...
begin
    w_debug('Inicio Borrado de la tabla ENEL_PREFACTURA_CEBP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_PREFACTURA_CEBP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_PREFACTURA_CEBP.', v_contador_debug);
    
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - inicio - 05.11.2021
    w_debug('Inicio Borrado de la tabla ENEL_AGR_FAC_PREFACTURA_CES.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES WHERE PERIODO = iperiod AND CANAL = 'BP' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    w_debug('Insertando Registros de CREDITOS en tabla ENEL_PREFACTURA_CEBP.' ,  v_contador_debug);
   
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CEBP ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, DESCRIPCION,
    ...
    FROM ENEL_CREDIT_TEMP_CEBP CREDTMP
        INNER JOIN ENEL_TXN_TEMP_CEBP TXNTMP 
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_PREFACTURA_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
    ...
    w_debug('Insertando Registros de INCENTIVOS en tabla ENEL_PREFACTURA_CEBP.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CEBP INCETMP
        INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
    
/* ==>> Solo para TVTA, resto de canales comentado
    w_debug('Insertando Registros de REMUN, precio BASE en tabla ENEL_PREFACTURA_CEBP.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CEBP ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, 
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CEBP para Precio BASE: '|| to_char(filas) || ' filas.', v_contador_debug);
*/
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PREFACTURA_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PREFACTURA_CEBP.',v_contador_debug);
end;
...
AS
begin
    w_debug('Inicio Borrado de la tabla ENEL_COMP_E4E_CEBP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_COMP_E4E_CEBP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_COMP_E4E_CEBP.', v_contador_debug);

    w_debug('Insertando Registros SCAWEB-E4E de datos en tabla ENEL_COMP_E4E_CEBP.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_COMP_E4E_CEBP ( PERIODO, IDPROVEEDOR, DESCRIPCION, ACTIVIDAD, PDS, NOMBRE_FISCAL, IMPORTE_SCAWEB, E4E_POS_CON_CONTRATO, 
    ...
    FROM
        ( select PERIODO, TRIM(to_char(PROVEEDOR,'000')) IDPROVEEDOR, SCA.CODIGO_AGENTE_INTERNO as PDS, sum(REALVALUE) as IMPORTE_SCAWEB, count(*) registros
            from ENEL_LIQUIDACION_CEBP sca where PERIODO = IPERIOD
    ...
    w_debug('Fin Carga Registros SCAWEB-E4E de la tabla ENEL_COMP_E4E_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando Registros E4E-SCAWEB (scaweb nulos) de datos en tabla ENEL_COMP_E4E_CEBP.' ,  v_contador_debug);
    ...
    FROM
        ...
        LEFT JOIN
            ( SELECT PERIODO
                ...
                , COUNT(*) REGISTROS        
                FROM ENEL_LIQUIDACION_CEBP SCA             
    ...
    w_debug('Fin Carga Registros E4E-SCAWEB (scaweb nulos) de la tabla ENEL_COMP_E4E_CEBP: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMP_E4E_CEBP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMP_E4E_CEBP.',v_contador_debug);
end; 
...
begin
    ...
    INSERT INTO ENELEXT.ENEL_AGREFACT_FINAL_CES (PERIODO, CANAL, PDS, LINEA_NEGOCIO, CONCEPTO_RETRIBUTIVO, TIPO_GASTO, VALUE, CECO, WBE_FINAL_IMPUTACION, COD_CONTRATO, DEPOSITSEQ, 
                                                ...
                                                FECHA_PREFACTURA, IMPORTE_CONTRATO_INI, RESTANTE, DET_ORDEN, CONCEPTO_DETALLE, CONSUMO_DEPOSITO,NOMBRE_INCENTIVO)
    SELECT 
    ...
    FROM ENEL_DEPOSIT_TEMP_CEBP DEPOTMP
        INNER JOIN ENEL_PDS_TEMP_CEBP TMP_PDS
    ...
    FROM ENEL_CREDIT_TEMP_CEBP CREDTMP
        ...
        INNER JOIN ENEL_COMMISSION_TEMP_CEBP COMMI
...
-- Inicio  Restante -- MRMM - 15/11/21
    ...
    v_count := 7;
    UPDATE ENEL_AGREFACT_FINAL_CES T1
    SET T1.RESTANTE =  RESTANTE - 
    (
        SELECT SUM(T2.VALUE) AS TOTAL
        FROM ENEL_DEPOSIT_TEMP_CEBP T2
        WHERE
             T1.DEPOSITSEQ = T2.DEPOSITSEQ
...
BEGIN    
    ...
    if  triggerFilename = 'EJECUCION_MANUAL' then
        ...
        /*BOM APM 14.12.2022 */
        w_debug('Periodo  '||v_periodo, v_contador_debug);
        w_debug('Processing Unit  '||v_puseq, v_contador_debug);
        /*EOM APM 14.12.2022 */
            ...
            --PREFACTURA CEBP
            UPDATE ENELEXT.ENEL_PREFACTURA_CEBP
    ...
    IF  v_PeriodoLiquidado = false THEN
        ...
        if(processingUnit='B2B CE BP') then
             ...
             IF f_ExisteInformeEnLista('E4E_BP', v_Listado_Informes) THEN
            ...
            ---------------------------------------
            IF f_ExisteInformeEnLista('LIQ_BP', v_Listado_Informes) THEN
                ...
                p_Actualiza_Informe_Fecha ( period, 'LIQ_BP');    
            end if;
            ...
            IF f_ExisteInformeEnLista('E4E_BP', v_Listado_Informes) THEN
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'E4E_BP');
                ...
                p_Actualiza_Informe_Fecha ( period, 'E4ENEG_BP');
            end if;

            ----------------------------
            -- Datos para Liquidacion --
            ----------------------------
            IF f_ExisteInformeEnLista('LIQFINAL_BP', v_Listado_Informes) THEN  
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'LIQFINAL_BP');
            end if;
            ...
            IF v_Listado_Informes = 'ALL' THEN
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'CANAL_BP');
            end if;        
     
            ---------------------------------------------------
            -- Datos para PREFACTURA
            ---------------------------------------------------
            IF f_ExisteInformeEnLista('PREFACT_BP', v_Listado_Informes) THEN
                -- Extraer datos para detalle de PreFactura
                p_Prefactura ( processingUnitSeq, period ,periodSeq , tenantId, v_Interfaz_Proceso );

                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'PREFACT_BP');   
            end if;
        end if;
