CREATE OR REPLACE PROCEDURE EXT.SMM_SP_ENTRADA_TIPO_OPERACION( IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio Truncado de la tabla SMM_ENTRADA_TIPO_SUBTIPO_TEMP.', v_log_count, v_idproceso,'info');
	EXECUTE IMMEDIATE 'TRUNCATE TABLE EXT.SMM_ENTRADA_TIPO_SUBTIPO_TEMP';
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin Truncado de la tabla SMM_ENTRADA_TIPO_SUBTIPO_TEMP.', v_log_count, v_idproceso,'info');
	
	
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Cargando tabla SMM_ENTRADA_TIPO_SUBTIPO_TEMP. Periodo: ' || i_period, v_log_count, v_idproceso,'info');
	INSERT INTO EXT.SMM_ENTRADA_TIPO_SUBTIPO_TEMP
	(
	    OCAP,ENTRADA,TIPO,SUBTIPO,UNIFICA,FLAG,NOMBRE_FISCAL,DELEGACION,UB,ID_OPERACION,EMPRESA,TIPO_PRESTADOR
	)
	SELECT
	    P.PDS,
	    O.ENTRADA,
	    O.TIPO,
	    O.SUBTIPO,
	    P.PDS || ' - ' || O.ENTRADA || ' - ' || O.TIPO || ' - ' || O.SUBTIPO,
	    'SI',
	    P.NOMBRE_FISCAL,
	    P.DELEGACION,
	    O.UB,
	    ...
	    O.EMPRESA,
	    P.TIPO_PRESTADOR
	FROM EXT.SMM_PDS_TEMP P
	CROSS JOIN
	(
	    SELECT DISTINCT
	        OPERACIONID      AS ID_OPERACION,
	        TIPO_ENTRADA     AS ENTRADA,
	        TIPO_OPERACION   AS TIPO,
	        SUBTIPO_OPERACION AS SUBTIPO,
	        UB,
	        EMPRESA
	    FROM EXT.SMM_OPERACIONES_UB_TEMP
	) O
	WHERE P.TIPO_POSICION IS NOT NULL;

    COMMIT;
    CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga Registros Créditos de la tabla SMM_ENTRADA_TIPO_SUBTIPO_TEMP: '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso,'info');
