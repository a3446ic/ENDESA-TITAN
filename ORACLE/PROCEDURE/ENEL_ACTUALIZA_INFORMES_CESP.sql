create or replace PACKAGE BODY         "ENEL_ACTUALIZA_INFORMES_CESP" AS
/* *****************************************************************************
	NAME:       ACTUALIZA_INFORMES_CESP
	PURPOSE:

	REVISIONS:
	Ver      		Date        	Author           	Description
	---------  		----------  	---------------  	-----------------------------------											
	1.0				15/06/2020		MPR	                Creacion de procedimientos para nueva processingUnit - CE_SP
    ...
    8.0             20/02/2026      APM                 Se modifica LEFT JOIN del insert a la tabla ENEL_DET_INCENTIVOS_CES
***************************************************************************** */
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CESP.', v_contador_debug);    
end;
...
begin
    ...
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_TXN_TEMP_CESP.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Borrado de la tabla ENEL_MEDIDAS_TEMP_CESP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_MEDIDAS_TEMP_CESP WHERE  ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_MEDIDAS_TEMP_CESP.', v_contador_debug);

    w_debug('Insertando datos en tabla ENEL_MEDIDAS_TEMP_CESP.' ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_MEDIDAS_TEMP_CESP (TENANTID,PERIODSEQ, PERIODO, NAME, VALUE, MEASUREMENTSEQ, PAYEESEQ, POSITIONSEQ, PIPELINERUNSEQ, PLANSEQ, RULESEQ)
...
/*      
  ...
  */              
    ...
    w_debug('Fin Carga de la tabla ENEL_MEDIDAS_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CESP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_CREDIT_TEMP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CESP( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
    ...
    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_CREDIT_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_CREDIT_TEMP_CESP.',v_contador_debug);
end;
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_COMMISSION_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_COMMISSION_TEMP_CESP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_COMMISSION_TEMP_CESP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_COMMISSION_TEMP_CESP( TENANTID, PERIODSEQ, PERIODO, COMMISSIONSEQ, CREDITSEQ, INCENTIVESEQ, ENTRYNUMBER, PIPELINERUNSEQ, PAYEESEQ, POSITIONSEQ, ORIGINTYPEID, PIPELINERUNDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_COMMISSION_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMMISSION_TEMP_CESP.',v_contador_debug);
end;
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_INCEN_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_INCEN_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_INCEN_TEMP_CESP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_INCEN_TEMP_CESP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_INCEN_TEMP_CESP(  TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, INCENTIVESEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE,
    ...
    w_debug('Fin Carga de la tabla ENEL_INCEN_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_INCEN_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_INCEN_TEMP_CESP.',v_contador_debug);
end;         
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_DEPOSIT_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_DEPOSIT_TEMP_CESP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_DEPOSIT_TEMP_CESP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_DEPOSIT_TEMP_CESP( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, DEPOSITSEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_DEPOSIT_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_DEPOSIT_TEMP_CESP.',v_contador_debug);   
end;         
...
begin
    w_debug('Inicio Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CESP.', v_contador_debug);
    ...
    w_debug('Fin Carga de la tabla ENEL_ORDER_IMPU_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_ORDER_IMPU_TEMP_CESP.',v_contador_debug); 
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CESP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_EQUIPAMIENTO_TEMP_CESP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CESP( TENANTID,PERIODSEQ,EQUIPAMIENTO, IDMARCA, MARCA, IDMODELO, MODELO, IDTIPO,
    ...
    w_debug('Fin Carga de la tabla ENEL_EQUIPAMIENTO_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CESP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CESP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CESP.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Origen de ENEL_E4E_NEGATIVOS_TEMP_CESP : CS_DEPOSIT.', v_contador_debug);
    ...
    INSERT INTO ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CESP( PERIODSEQ,PERIODO, DEPOSITSEQ,POSITIONSEQ,PAYEESEQ,VALUE,PDS,
    ...
    FROM ENEL_DEPOSIT_TEMP_CESP DEPO
        ...
        LEFT JOIN ENEL_E4E_CONTRATOS_TEMP_CESP TMP_CONTRA
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CESP.', v_contador_debug);
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_CONTRATOS_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CESP.',v_contador_debug);   
end;
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_PDS_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_PDS_TEMP_CESP';
    ...
    w_debug('Fin Carga de la tabla ENEL_PDS_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PDS_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PDS_TEMP_CESP.',v_contador_debug);
end;
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CESP.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CESP';
    w_debug('Fin Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CESP.', v_contador_debug);

    w_debug('Cargando tabla ENEL_E4E_DEPOSIT_TEMP_CESP. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    -- Si se ejecuta en una FASE que no es REWARD Utilizamos la tabla de PAGOS
    w_debug('Origen de ENEL_E4E_DEPOSIT_TEMP_CESP.', v_contador_debug);

	INSERT INTO ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CESP( PERIODSEQ, PERIODO, POSITIONSEQ,PAYEESEQ,VALUE,PDS,PAR_PROVEEDOR,TIPO_IMPOSITIVO,COD_CONTRATO,POS_DOC,
												...
												SUBPOSICION, BUSINESSUNIT, LINEA_NEGOCIO, TIPO, CAMPANIA, NOM_SOLICITANTE, POS_ORDEN, LINEA_SERVICIO,GENERICDATE1,GENERICATTRIBUTE9) --DMS 28.06.2024
    Select
        DEPO.PERIODSEQ,
		iperiod,
        DEPO.POSITIONSEQ, 
    ...
    FROM  ENELEXT.enel_deposit_temp_CESP DEPO				 
		...
		INNER JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CESP TMP_PROV 
			ON TMP_PROV.IDPROVEEDOR=depo.earninggroupid  
			AND depo.periodseq=TMP_PROV.periodseq

		LEFT JOIN ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CESP TMP_CONTRA
			ON TMP_CONTRA.periodseq=depo.periodseq
			AND TMP_CONTRA.PDS = TMP_PDS.PDS
			AND TMP_CONTRA.ACTIVIDAD_DETALLADA=TMP_PROV.DETALLE_ACTIVIDAD
			AND TMP_CONTRA.periodseq=TMP_PROV.periodseq
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_DEPOSIT_TEMP_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CESP COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CESP.',v_contador_debug);
end;
...
AS
    contador integer;  
begin
    w_debug('Inicio Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CESP para fichero ' || iFichero ,  v_contador_debug);
    contador := 1;
...
begin
	...
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CESP WHERE PERIODO = iperiod AND FICHERO like 'E%1' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
    ...
    BEGIN
		LOOP
			DELETE FROM ENEL_AGR_FAC_E4E_FINAL_CES WHERE PERIODO = iperiod AND CANAL = 'SP' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;    
    ...
    DECLARE
        CURSOR C_TMPDEPOSITOS IS
            ...
            FROM ENEL_E4E_DEPOSIT_TEMP_CESP
...
begin
	...
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CESP WHERE PERIODO = iperiod AND FICHERO like 'E%2'  AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
...
begin
    w_debug('Inicio Borrado de la tabla ENEL_LIQUIDACION_CESP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_LIQUIDACION_CESP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_LIQUIDACION_CESP: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando INCENTIVOS de datos en tabla ENEL_LIQUIDACION_CESP.' ,  v_contador_debug);
...
begin
    ...
    FROM ENEL_TXN_TEMP_CESP ETT
        ...
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CESP TEMP_PROV
    ...
    SELECT 
        CASE 
        WHEN ince.name = 'C - CE - SP - Electricidad - Incentivo Objetivos (MWh) Trimestral NN 0'
			or ince.name = 'C - CE - SP - Electricidad - Incentivo Objetivos (MWh) Trimestral NN -1'
			or ince.name = 'C - CE - SP - Electricidad - Incentivo Objetivos (MWh) Trimestral NN -2'
			or ince.name = 'C - CE - SP - Electricidad - Incentivo Objetivos (MWh) Trimestral NN -3'
			or ince.name = 'C - CE - SP - Electricidad - Incentivo Objetivos (MWh) Trimestral NN -4'
        THEN 
            CASE 
    ...
    FROM ENEL_TXN_TEMP_CESP ETT
		...
		INNER JOIN cs_commission commi
            ...
            /*BOM APM 20.02.2026 Old Code*/
            --LEFT JOIN ENEL_INCEN_TEMP_CEIS INCE
            --New Code
            LEFT JOIN ENEL_INCEN_TEMP_CESP INCE
            /*EOM APM 20.02.2026 Old Code*/
...
AS
    ...
    v_mesanio VARCHAR(4);
begin
    w_debug('Inicio Borrado de la tabla ENEL_PREFACTURA_CESP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_PREFACTURA_CESP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES WHERE PERIODO = iperiod AND CANAL = 'SP' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    SELECT 
        ...
        CREDTMP.BU_NAME || v_txtMes_Liquidacion2 || TMP_PDS.PDS ||
			CASE 
				WHEN credtmp.GenericAttribute1 like 'I%' THEN 'I' || SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13, 1, 1)
				WHEN credtmp.GenericAttribute1 like 'PF' and CREDTMP.BU_NAME = 'CEBP' THEN 'PF' -- NO MODIFICAR
				WHEN SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13, 1, 1) = 'B' THEN SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13, 1, 1)
				WHEN SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13, 1, 1) = 'D' THEN SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13, 1, 1)
				ELSE SUBSTR(TXNTMP.TEX0_GENERICATTRIBUTE13,0,1) || SUBSTR(credtmp.GenericAttribute1,0,1)
			END as PDS,        
		CREDTMP.value as IMPORTE,
    ...
    SELECT 
		INCETMP.PERIODO, 
        INCETMP.BU_NAME || v_txtMes_Liquidacion2 || TMP_PDS.PDS ||
			CASE 
				WHEN INCETMP.GenericAttribute1 like 'I%' THEN 'I' || SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1)
				WHEN INCETMP.GenericAttribute1 like 'PF' and INCETMP.BU_NAME = 'CEBP' THEN 'PF' -- NO MODIFICAR
				WHEN SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1) = 'B' THEN SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1)
				WHEN SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1) = 'D' THEN SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1)
				ELSE SUBSTR(INCETMP.GENERICATTRIBUTE2,1,1) || SUBSTR(INCETMP.GenericAttribute1,0,1)
			END,    
		INCETMP.value,
        TMP_PDS.PDS || SUBSTR(TMP_PROV.DESCRIPCION, 1, 1) || SUBSTR(TMP_PROV.DESCRIPCION, INSTR(TMP_PROV.DESCRIPCION, ' ')+1, 1) ||
        CASE
...
*/
    ...
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PREFACTURA_CESP.',v_contador_debug);
end;
...
AS
begin
    w_debug('Inicio Borrado de la tabla ENEL_COMP_E4E_CESP.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_COMP_E4E_CESP WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    FROM
        ...
        LEFT JOIN
            ( SELECT PERIODO
                ...
                , COUNT(*) REGISTROS        
                FROM ENEL_LIQUIDACION_CESP SCA             
...
begin
    ...
    SELECT 
		...
		case
            ...
            when DEPOTMP.TIPO = 'PB' and DEPOTMP.value < 0 and DEPOTMP.name like '%Decomi%' then 'Decomision' -- DCR 25.02.2022
    ...
    FROM ENEL_DEPOSIT_TEMP_CESP DEPOTMP
        INNER JOIN ENEL_PDS_TEMP_CESP TMP_PDS
	...
	(
		...
		WHERE
			...
			AND SUBSTR(T1.LINEA_NEGOCIO, 1, 1) = SUBSTR(T2.LINEA_NEGOCIO, 1, 1)
           	AND SUBSTR(T1.TIPO_GASTO, 1, 1) = SUBSTR(T2.TIPO_GASTO, 1, 1)
			AND T1.CONCEPTO_RETRIBUTIVO = T2.CONCEPTO_RETRIBUTIVO
            AND T2.NAME_REGLA NOT LIKE '%Incentivo%' --APM 30.04.2024            
	)
	WHERE T1.PERIODO = iperiod
...
BEGIN    
    ...
    IF  v_PeriodoLiquidado = false THEN
		...
		if(processingUnit='B2B CE SP') then
			...
			---------------------------------------
			IF f_ExisteInformeEnLista('LIQ_SP', v_Listado_Informes) THEN
            ...
            IF f_ExisteInformeEnLista('E4E_SP', v_Listado_Informes) THEN
				...
				p_Actualiza_Informe_Fecha ( period, 'E4ENEG_SP');
            end if;

            ----------------------------
            -- Datos para Liquidacion --
            ----------------------------
            IF f_ExisteInformeEnLista('LIQFINAL_SP', v_Listado_Informes) THEN  
            ...
            ---------------------------------------------------
            IF f_ExisteInformeEnLista('PREFACT_SP', v_Listado_Informes) THEN
