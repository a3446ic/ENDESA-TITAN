CREATE PROCEDURE EXT.SMM_SP_FINAL_E4E_NEGATIVOS_CAT_TVTA( IN i_processingUnitSeq BIGINT ,IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_E4E_NEGATIVOS_CAT_TVTA.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_E4E_NEGATIVOS_CAT_TVTA WHERE PERIODO = i_period;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_E4E_NEGATIVOS_CAT_TVTA.', v_log_count, v_idproceso,'info');
	...
	INTO v_maxIDPEDIDO
	FROM EXT.SMM_E4E_NEGATIVOS_CAT_TVTA
    ...
    CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Insertando Registros de datos en tabla EXT.SMM_E4E_NEGATIVOS_CAT_TVTA.' || v_codFichero , v_log_count, v_idproceso,'info');
    
	INSERT INTO EXT.SMM_E4E_NEGATIVOS_CAT_TVTA ( PERIODSEQ, PERIODO, DEPOSITSEQ, POSITIONSEQ, PAYEESEQ, POS_NAME, IDPEDIDO, ORG_VENTAS, CANAL_DISTRIBUCION, 
	...
	SELECT 
        ...
        --New Code    
        WHEN e4edt.ACTIVIDAD = 'CAPTACIÓN' THEN v_txtYear || e4edt.ID_CENTRO_E4E || 'CAT' || v_codMes
			WHEN e4edt.ACTIVIDAD = 'RECUPERACIÓN' THEN v_txtYear || e4edt.ID_CENTRO_E4E || 'REC' || v_codMes
			WHEN e4edt.ACTIVIDAD = 'MKT DIRECTO COTEL' THEN v_txtYear || e4edt.ID_CENTRO_E4E || 'MKT' || v_codMes
			ELSE v_txtYear || e4edt.POS_NAME || 'ERR' || v_codMes
        /*EOM APM 21.05.2026*/    
		END NUM_PEDIDO,
		'' FechaPedido,
	...
	FROM EXT.SMM_E4E_NEG_TEMP_CAT_TVTA e4edt 
	WHERE 
		e4edt.VALUE < 0  
		and e4edt.PERIODSEQ = i_periodseq;
               
    
     COMMIT;
        
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_E4E_NEGATIVOS_CAT_TVTA '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
