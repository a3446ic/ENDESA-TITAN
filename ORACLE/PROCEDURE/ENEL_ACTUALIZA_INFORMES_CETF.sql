create or replace PACKAGE BODY         "ENEL_ACTUALIZA_INFORMES_CETF" AS
/* *****************************************************************************
	NAME:       ACTUALIZA_INFORMES_CETF
	PURPOSE:

	REVISIONS:
	Ver      		Date        	Author           	Description
	---------  		----------  	---------------  	-----------------------------------											
	1.0				15/06/2020		MPR	                Creacion de procedimientos para nueva processingUnit - CE_TF
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CETF.', v_contador_debug);    
end;
...
begin
    w_debug('Cargando tabla ENEL_TXN_TEMP_CETF. Periodo:'|| iperiod ,  v_contador_debug);
	v_periodstartdate :=  f_fecha_inicio(iperiodseq);
	v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
	
    INSERT INTO ENELEXT.ENEL_TXN_TEMP_CETF( TENANTID, PERIODSEQ, PERIODO, SALESORDERSEQ, SALESTRANSACTIONSEQ, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTYPEID, COMPENSATIONDATE, ACCOUNTINGDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_TXN_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    BEGIN
        SYS.DBMS_STATS.GATHER_TABLE_STATS (
        OwnName => 'ENELEXT'
        ,TabName => 'ENEL_TXN_TEMP_CETF'
        ,Estimate_Percent => 1
    ...
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_TXN_TEMP_CETF.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Borrado de la tabla ENEL_MEDIDAS_TEMP_CETF.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_MEDIDAS_TEMP_CETF WHERE  ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_MEDIDAS_TEMP_CETF.', v_contador_debug);

    w_debug('Insertando datos en tabla ENEL_MEDIDAS_TEMP_CETF.' ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_MEDIDAS_TEMP_CETF (TENANTID,PERIODSEQ, PERIODO, NAME, VALUE, MEASUREMENTSEQ, PAYEESEQ, POSITIONSEQ, PIPELINERUNSEQ, PLANSEQ, RULESEQ)
...
/*      
  ...
  */              
    ...
    w_debug('Fin Carga de la tabla ENEL_MEDIDAS_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_MEDIDAS_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_MEDIDAS_TEMP_CETF',v_contador_debug);
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_CREDIT_TEMP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CETF( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
    ...
    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_CREDIT_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_CREDIT_TEMP_CETF.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_COMMISSION_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_COMMISSION_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_COMMISSION_TEMP_CETF. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_COMMISSION_TEMP_CETF( TENANTID, PERIODSEQ, PERIODO, COMMISSIONSEQ, CREDITSEQ, INCENTIVESEQ, ENTRYNUMBER, PIPELINERUNSEQ, PAYEESEQ, POSITIONSEQ, ORIGINTYPEID, PIPELINERUNDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_COMMISSION_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMMISSION_TEMP_CETF.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_INCEN_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_INCEN_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_INCEN_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_INCEN_TEMP_CETF. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_INCEN_TEMP_CETF(  TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, INCENTIVESEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE,
    ...
    w_debug('Fin Carga de la tabla ENEL_INCEN_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_INCEN_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_INCEN_TEMP_CETF.',v_contador_debug);
end;         
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_DEPOSIT_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_DEPOSIT_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_DEPOSIT_TEMP_CETF. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_DEPOSIT_TEMP_CETF( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, DEPOSITSEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_DEPOSIT_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_DEPOSIT_TEMP_CETF.',v_contador_debug);   
end;         
...
begin
    w_debug('Inicio Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CETF.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_ORDER_IMPU_TEMP_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF( TENANTID,PERIODSEQ,IDPROVEEDOR,DESCRIPCION,DESCRIPCION_CORTA, FICHERO,CECO,
    ...
    w_debug('Fin Carga de la tabla ENEL_ORDER_IMPU_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF.',v_contador_debug); 
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CETF.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_EQUIPAMIENTO_TEMP_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CETF( TENANTID,PERIODSEQ,EQUIPAMIENTO, IDMARCA, MARCA, IDMODELO, MODELO, IDTIPO,
    ...
    w_debug('Fin Carga de la tabla ENEL_EQUIPAMIENTO_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CETF.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CETF WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CETF.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Origen de ENEL_E4E_NEGATIVOS_TEMP_CETF : CS_DEPOSIT.', v_contador_debug);
    ...
    INSERT INTO ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CETF( PERIODSEQ,PERIODO, DEPOSITSEQ,POSITIONSEQ,PAYEESEQ,VALUE,PDS,
    ...
    FROM ENEL_DEPOSIT_TEMP_CETF DEPO
        INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS 
        ...
        LEFT JOIN ENEL_E4E_CONTRATOS_TEMP_CETF TMP_CONTRA
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_NEGATIVOS_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CETF.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_E4E_CONTRATOS_TEMP_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CETF(TENANTID,PERIODSEQ,ID,PDS,ACTIVIDAD_DETALLADA,ACTIVIDAD, CIF,COD_CONTRATO,POS_DOC,TEXTO_BREVE,CODIGO_SERVICIO,
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_CONTRATOS_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CETF.',v_contador_debug);   
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_PDS_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_PDS_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_PDS_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_PDS_TEMP_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_PDS_TEMP_CETF( PERIODSEQ, RULEELEMENTOWNERSEQ, PAYEESEQ, PAYEEID, PDS, NOMBRE_FISCAL, CIF, NOMBRE_CUENTA, CALLE, COD_POSTAL, 
    ...
    w_debug('Fin Carga de la tabla ENEL_PDS_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PDS_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PDS_TEMP_CETF.',v_contador_debug);
end;
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CETF.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CETF';
    w_debug('Fin Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CETF.', v_contador_debug);

    w_debug('Cargando tabla ENEL_E4E_DEPOSIT_TEMP_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    -- Si se ejecuta en una FASE que no es REWARD Utilizamos la tabla de PAGOS
    w_debug('Origen de ENEL_E4E_DEPOSIT_TEMP_CETF.', v_contador_debug);

	INSERT INTO ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CETF( PERIODSEQ, PERIODO, POSITIONSEQ,PAYEESEQ,VALUE,PDS,PAR_PROVEEDOR,TIPO_IMPOSITIVO,COD_CONTRATO,POS_DOC,
    ...
    FROM  ENELEXT.enel_deposit_temp_CETF DEPO				 
		INNER JOIN ENELEXT.ENEL_PDS_TEMP_CETF TMP_PDS 
			ON depo.payeeseq=TMP_PDS.payeeseq 
			and depo.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
			and depo.periodseq=TMP_PDS.periodseq

		INNER JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF TMP_PROV 
			ON TMP_PROV.IDPROVEEDOR=depo.earninggroupid  
			AND depo.periodseq=TMP_PROV.periodseq

		LEFT JOIN ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CETF TMP_CONTRA
			ON TMP_CONTRA.periodseq=depo.periodseq
			AND TMP_CONTRA.PDS = TMP_PDS.PDS
			AND TMP_CONTRA.ACTIVIDAD_DETALLADA=TMP_PROV.DETALLE_ACTIVIDAD
			AND TMP_CONTRA.periodseq=TMP_PROV.periodseq
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_DEPOSIT_TEMP_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CETF.',v_contador_debug);
end;
...
AS
    contador integer;  
begin
    w_debug('Inicio Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CETF para fichero ' || iFichero ,  v_contador_debug);
    contador := 1;
    -- Registro de CABECERA 1 : lista de campos
    INSERT INTO ENEL_E4E_FINAL_CETF (PERIODO, ORDEN, CAMPO1, CAMPO2, CAMPO3, CAMPO4, CAMPO5, CAMPO6, CAMPO7, CAMPO8, CAMPO9, CAMPO10, 
    ...
    COMMIT;
    w_debug('Fin Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CETF para fichero ' || iFichero ,  v_contador_debug);
...
AS
	...
	v_aux VARCHAR(10);
begin
    w_debug('Inicio Borrado de la tabla ENEL_E4E_FINAL_CETF.', v_contador_debug);
   
    --v_codFichero :='E4E1';  -- v2.0 se asigna el valor dinamicamente en funcion de la actividad del proveedor
        
	-- EXECUTE IMMEDIATE 'DELETE ENELEXT.ENEL_E4E_FINAL_CETF WHERE ....';
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CETF WHERE PERIODO = iperiod AND FICHERO like 'E%1' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
    w_debug('Fin Borrado de la tabla ENEL_E4E_FINAL_CETF.', v_contador_debug);
    ...
    BEGIN
		LOOP
			DELETE FROM ENEL_AGR_FAC_E4E_FINAL_CES WHERE PERIODO = iperiod AND CANAL = 'TF' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;    
    ...
    w_debug('Cargando tabla ENEL_E4E_FINAL_CETF. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    w_debug('Insertando Registros de datos en tabla ENEL_E4E_FINAL_CETF. Fichero ' || v_codFichero ,  v_contador_debug);
    ...
    DECLARE
        CURSOR C_TMPDEPOSITOS IS
            ...
            FROM ENEL_E4E_DEPOSIT_TEMP_CETF
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CETF:  E4E1 '|| to_char(contadorE4E) || ' -- ECS1 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
end;
...
begin
	...
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CETF WHERE PERIODO = iperiod AND FICHERO like 'E%2'  AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CETF:  E4E2 '|| to_char(contadorE4E) || ' -- ECS2 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
...
begin
    ...
    FROM ENEL_E4E_NEGATIVOS_TEMP_CETF e4edt 
...
begin
    w_debug('Inicio Borrado de la tabla ENEL_LIQUIDACION_CETF.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_LIQUIDACION_CETF WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_LIQUIDACION_CETF.', v_contador_debug);
    ...
    w_debug('Insertando CREDITOS de datos en tabla ENEL_LIQUIDACION_CETF.' ,  v_contador_debug);
    -- v2.0 Se cambia la tabla de origen CS_CREDIT  a la temporal ENEL_CREDIT_TEMP
    INSERT INTO ENELEXT.ENEL_LIQUIDACION_CETF ( PERIODO, PROVEEDOR, ANO_LIQUIDACION, MES_LIQUIDACION, CODIGO_AGENTE_INTERNO, CONCEPTO, CANTIDAD, 
    ...
    FROM ENEL_CREDIT_TEMP_CETF credtmp
        INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS 
            ON credtmp.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
		LEFT JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CETF TMP_PROV
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_LIQUIDACION_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando INCENTIVOS de datos en tabla ENEL_LIQUIDACION_CETF.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CETF INCENTMP    
        INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_LIQUIDACION_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_LIQUIDACION_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_LIQUIDACION_CETF.',v_contador_debug);
end;
...
begin
    ...
    FROM ENEL_TXN_TEMP_CETF ETT
        LEFT JOIN ENEL_CREDIT_TEMP_CETF ECT
            ON ECT.SALESTRANSACTIONSEQ = ETT.SALESTRANSACTIONSEQ
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CETF TEMP_PROV
            ON TEMP_PROV.IDPROVEEDOR=ECT.GENERICATTRIBUTE4
		LEFT JOIN ENEL_PDS_TEMP_CETF TMP_PDS 
...
begin
    ...
    from ENELEXT.ENEL_INCEN_TEMP_CETF edt
		INNER JOIN ENEL_PDS_TEMP_CETF CSP
...
begin
    ...
    FROM ENELEXT.ENEL_INCEN_TEMP_CETF CSI
       	INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS
...
AS
    ...
    v_mesanio VARCHAR(4);
begin
    w_debug('Inicio Borrado de la tabla ENEL_PREFACTURA_CETF.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_PREFACTURA_CETF WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_PREFACTURA_CETF.', v_contador_debug);
    
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - inicio - 05.11.2021
    w_debug('Inicio Borrado de la tabla ENEL_AGR_FAC_PREFACTURA_CES.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES WHERE PERIODO = iperiod AND CANAL = 'TF' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    ...
    w_debug('Insertando Registros de CREDITOS en tabla ENEL_PREFACTURA_CETF.' ,  v_contador_debug);
   
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CETF ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, DESCRIPCION,
    ...
    FROM ENEL_CREDIT_TEMP_CETF CREDTMP
        INNER JOIN ENEL_TXN_TEMP_CETF TXNTMP 
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_PREFACTURA_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
    ...
    w_debug('Insertando Registros de INCENTIVOS en tabla ENEL_PREFACTURA_CETF.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CETF INCETMP
        INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
...
/* ==>> Solo para TVTA, resto de canales comentado
	w_debug('Insertando Registros de REMUN, precio BASE en tabla ENEL_PREFACTURA_CETF.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CETF ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, 
	...
	w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CETF para Precio BASE: '|| to_char(filas) || ' filas.', v_contador_debug);
*/
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PREFACTURA_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PREFACTURA_CETF.',v_contador_debug);
end;
...
AS
begin
    w_debug('Inicio Borrado de la tabla ENEL_COMP_E4E_CETF.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_COMP_E4E_CETF WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_COMP_E4E_CETF.', v_contador_debug);

    w_debug('Insertando Registros SCAWEB-E4E de datos en tabla ENEL_COMP_E4E_CETF.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_COMP_E4E_CETF ( PERIODO, IDPROVEEDOR, DESCRIPCION, ACTIVIDAD, PDS, NOMBRE_FISCAL, IMPORTE_SCAWEB, E4E_POS_CON_CONTRATO, 
    ...
    FROM
        ( select PERIODO, TRIM(to_char(PROVEEDOR,'000')) IDPROVEEDOR, SCA.CODIGO_AGENTE_INTERNO as PDS, sum(REALVALUE) as IMPORTE_SCAWEB, count(*) registros
            from ENEL_LIQUIDACION_CETF sca where PERIODO = IPERIOD
    ...
    w_debug('Fin Carga Registros SCAWEB-E4E de la tabla ENEL_COMP_E4E_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando Registros E4E-SCAWEB (scaweb nulos) de datos en tabla ENEL_COMP_E4E_CETF.' ,  v_contador_debug);
    ...
    FROM
        ...
        LEFT JOIN
            ( SELECT PERIODO
                ...
                , COUNT(*) REGISTROS        
                FROM ENEL_LIQUIDACION_CETF SCA             
    ...
    w_debug('Fin Carga Registros E4E-SCAWEB (scaweb nulos) de la tabla ENEL_COMP_E4E_CETF: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMP_E4E_CETF COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMP_E4E_CETF.',v_contador_debug);
end; 
...
begin
    ...
    FROM ENEL_DEPOSIT_TEMP_CETF DEPOTMP
        INNER JOIN ENEL_PDS_TEMP_CETF TMP_PDS
	...
	FROM ENEL_CREDIT_TEMP_CETF CREDTMP
		...
		INNER JOIN ENEL_COMMISSION_TEMP_CETF COMMI
...
-- DCR EOM 29.12.2021 INC000079731266 
	w_debug('Fin Update tabla ENEL_AGREFACT_FINAL_CES.', v_count); --APM 13.04.2022
    ...
    EXCEPTION
        WHEN OTHERS THEN
            w_debug('ERROR en actualizacion de columnas ALEGACIONES o N REGISTROS. Línea:' || v_count, v_contador_debug);
...
BEGIN    
    ...
    if  triggerFilename = 'EJECUCION_MANUAL' then
        ...
        /*BOM APM 14.12.2022 */
        w_debug('CETF - Periodo  '||v_periodo, v_contador_debug);
        ...
        /*EOM APM 14.12.2022 */
            ...
            --PREFACTURA CETF
            UPDATE ENELEXT.ENEL_PREFACTURA_CETF
    ...
    IF  v_PeriodoLiquidado = false THEN
		...
		if(processingUnit='B2B CE TF') then
            ...
            IF f_ExisteInformeEnLista('E4E_TF', v_Listado_Informes) THEN
			...
			---------------------------------------
			IF f_ExisteInformeEnLista('LIQ_TF', v_Listado_Informes) THEN
				...
				p_Actualiza_Informe_Fecha ( period, 'LIQ_TF');    
			end if;
            ...
            ----------------------------
            IF f_ExisteInformeEnLista('LIQFINAL_TF', v_Listado_Informes) THEN  
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'LIQFINAL_TF');
            end if;
            ...
            IF v_Listado_Informes = 'ALL' THEN
				...
				-- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'CANAL_TF');
            end if;        
     
            ---------------------------------------------------
            -- Datos para PREFACTURA
            ---------------------------------------------------
            IF f_ExisteInformeEnLista('PREFACT_TF', v_Listado_Informes) THEN
                -- Extraer datos para detalle de PreFactura
                p_Prefactura ( processingUnitSeq, period ,periodSeq , tenantId, v_Interfaz_Proceso );

                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'PREFACT_TF');   
            end if;
