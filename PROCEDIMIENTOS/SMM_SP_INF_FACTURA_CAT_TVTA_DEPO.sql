CREATE PROCEDURE EXT.SMM_SP_INF_FACTURA_CAT_TVTA_DEPO ( IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_FACTCAT_TVTA_DEPOSITOS.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_FACTCAT_TVTA_DEPOSITOS WHERE PERIODSEQ = i_periodseq;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_FACTCAT_TVTA_DEPOSITOS.', v_log_count, v_idproceso,'info');
	


   INSERT INTO EXT.SMM_FACTCAT_TVTA_DEPOSITOS (TENANTID, PERIODSEQ, POSITIONSEQ, EARNINGGROUPID, IMPORTE)
	SELECT 
		TENANTID,
		PERIODSEQ,
		POSITIONSEQ,
		EARNINGGROUPID,
		SUM(VALUE)
	FROM EXT.SMM_DEPOSIT_TEMP_CAT_TVTA
	GROUP BY 
		TENANTID,
		PERIODSEQ,
		POSITIONSEQ,
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_FACTCAT_TVTA_DEPOSITOS '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
