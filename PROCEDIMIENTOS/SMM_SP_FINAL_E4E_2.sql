CREATE OR REPLACE PROCEDURE EXT.SMM_SP_FINAL_E4E_2( IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	DELETE FROM EXT.SMM_E4E_FINAL WHERE PERIODO = i_period AND FICHERO LIKE 'E%2';
		...
		TEMP_FINAL =
			SELECT
			    ...
			    CASE t.ACTIVIDAD
			        WHEN 'STP' THEN 'E4E2'
			        WHEN 'SSII' THEN 'E4E2'
			        WHEN 'CAPTACIÓN' THEN 'ECS2'
			        WHEN 'CAPTACION' THEN 'ECS2'
			        WHEN 'ATC' THEN 'ECS2'
			        ELSE 'NOT2'
			    END AS COD_FICHERO,
    ...
    */
    ...
    if contadorE4E > 4 THEN
		CALL EXT.SMM_SP_CABECERA_FICHEROS_E4E(  i_period , 'E4E2' );
	end if;
    ...
    if contadorECS > 4 THEN
		CALL EXT.SMM_SP_CABECERA_FICHEROS_E4E(  i_period , 'ECS2' );
	end if;   
        
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_E4E_FINAL:  E4E2'|| to_char(contadorE4E) || ' -- ECS2'|| to_char(contadorECS) || ' filas.', v_log_count, v_idproceso, 'info');
