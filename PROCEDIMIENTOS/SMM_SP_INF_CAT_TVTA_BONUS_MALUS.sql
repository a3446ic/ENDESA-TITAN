CREATE PROCEDURE EXT.SMM_SP_INF_CAT_TVTA_BONUS_MALUS( IN i_processingUnitSeq BIGINT ,IN i_period VARCHAR(25), IN i_periodseq BIGINT, IN i_interfaz NVARCHAR(50))
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	--SMM_CAT_TVTA_BONUS_MALUS
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_CAT_TVTA_BONUS_MALUS.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_CAT_TVTA_BONUS_MALUS WHERE PERIODO = i_period;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_CAT_TVTA_BONUS_MALUS.', v_log_count, v_idproceso,'info');
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Insertando Registros de datos en tabla EXT.SMM_CAT_TVTA_BONUS_MALUS.' ,v_log_count, v_idproceso,'info');
    
	INSERT INTO EXT.SMM_CAT_TVTA_BONUS_MALUS ( PERIODSEQ, POSITIONSEQ, PAYEESEQ, PERIODO, POSICION, NOMBRE_MEDIDA, REALIZADO_QUOTASAT, OBJETIVO_TOTALBM,
	...
	FROM EXT.SMM_CAT_TVTA_BONUS_MALUS_TEMP
    WHERE 
		PERIODO =  i_period;
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_CAT_TVTA_BONUS_MALUS '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
