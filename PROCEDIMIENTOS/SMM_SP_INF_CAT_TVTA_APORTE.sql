CREATE PROCEDURE EXT.SMM_SP_INF_CAT_TVTA_APORTE( IN i_processingUnitSeq BIGINT ,IN i_period VARCHAR(25), IN i_periodseq BIGINT, IN i_interfaz NVARCHAR(50))
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	--SMM_CAT_TVTA_APORTE
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_CAT_TVTA_APORTE.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_CAT_TVTA_APORTE WHERE PERIODO = i_period;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_CAT_TVTA_APORTE.', v_log_count, v_idproceso,'info');
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Insertando Registros de datos en tabla EXT.SMM_CAT_TVTA_APORTE.' ,v_log_count, v_idproceso,'info');
    
	INSERT INTO EXT.SMM_CAT_TVTA_APORTE ( PAYEESEQ, POSITIONSEQ, PERIODSEQ, PERIODO, CODIGO_COMERCIAL, CANAL, N_REGISTROS, APORTE_UNITARIO, APORTE_TOTAL, ID_PRODUCTO, 
	...
	FROM EXT.SMM_CAT_TVTA_APORTE_TEMP
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_CAT_TVTA_APORTE '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
