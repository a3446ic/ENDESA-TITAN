create or replace PACKAGE BODY         "ENEL_ACTUALIZA_INFORMES_CEIS" AS
/* *****************************************************************************
	NAME:       ACTUALIZA_INFORMES_CEIS
	PURPOSE:

	REVISIONS:
	Ver      		Date        	Author           	Description
	---------  		----------  	---------------  	-----------------------------------											
	1.0				15/06/2020		MPR	                Creacion de procedimientos para nueva processingUnit - CE_IS
    2.0             07/02/2022      DCR                 Informe Agregado no coherente con cuadre de liquidación
    3.0             05/06/2023      APM                 Campo CONTRATO en Informe Cuadre Liquidación (pestañas Precio Base Variable y Detalle Icentivos CUPS
	4.0             29/05/2024      APM                 Se añade nuevo campo Decomisión a la tabla ENEL_CUADRELIQ_FINAL_CES
    5.0             11/07/2024      APM                 Actualizar CAMPO11 a 2 decimales.
    6.0             18/07/2024      APM                 Añadir nuevo campo TXN.GB2 (SS_GARANTIA) en la tabla ENEL_CUADRELIQ_FINAL_CES.
    7.0             05/11/2024      APM                 Añadir nuevo campo TXN.GA1 (TIPO_SERVICIO) en la tabla ENEL_CUADRELIQ_FINAL_CES.     
    8.0             07/11/2024      APM                 Añadir nuevo campo TXN.GB1 (GESTION_CARTERA) en la tabla ENEL_CUADRELIQ_FINAL_CES.
...
AS
begin
    w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CEIS.', v_contador_debug);    
	
	w_debug('Inicio Truncado de la tabla ENEL_TXN_TEMP_CEIS_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_TXN_TEMP_CEIS_2';
    w_debug('Fin Truncado de la tabla ENEL_TXN_TEMP_CEIS_2.', v_contador_debug);   
end;
...
begin
    w_debug('Cargando tabla ENEL_TXN_TEMP_CEIS. Periodo:'|| iperiod ,  v_contador_debug);
	v_periodstartdate :=  f_fecha_inicio(iperiodseq);
	v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
	
    INSERT INTO ENELEXT.ENEL_TXN_TEMP_CEIS( TENANTID, PERIODSEQ, PERIODO, SALESORDERSEQ, SALESTRANSACTIONSEQ, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTYPEID, COMPENSATIONDATE, ACCOUNTINGDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_TXN_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    BEGIN
        SYS.DBMS_STATS.GATHER_TABLE_STATS (
        OwnName => 'ENELEXT'
        ,TabName => 'ENEL_TXN_TEMP_CEIS'
        ,Estimate_Percent => 1
    ...
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_TXN_TEMP_CEIS.',v_contador_debug);
	
	w_debug('Cargando tabla ENEL_TXN_TEMP_CEIS_2. Periodo:'|| iperiod ,  v_contador_debug);
	v_periodstartdate :=  f_fecha_inicio(iperiodseq);
	v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);
	
	INSERT INTO ENELEXT.ENEL_TXN_TEMP_CEIS_2( TENANTID, PERIODSEQ, PERIODO, SALESORDERSEQ, SALESTRANSACTIONSEQ, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTYPEID, COMPENSATIONDATE, ACCOUNTINGDATE, 
                                        PRODUCTID, GENERICATTRIBUTE1, GENERICATTRIBUTE2, GENERICATTRIBUTE3, GENERICATTRIBUTE4, GENERICATTRIBUTE5, GENERICATTRIBUTE6, GENERICATTRIBUTE7, 
                                        GENERICATTRIBUTE8, GENERICATTRIBUTE9, GENERICATTRIBUTE10, GENERICATTRIBUTE11, GENERICATTRIBUTE12, GENERICATTRIBUTE13, GENERICATTRIBUTE14, 
                                        GENERICATTRIBUTE15, GENERICATTRIBUTE16, GENERICATTRIBUTE17, GENERICATTRIBUTE18,GENERICATTRIBUTE19, GENERICATTRIBUTE20, 
                                        GENERICATTRIBUTE21, GENERICNUMBER1, GENERICNUMBER2, GENERICNUMBER3, GENERICDATE3, GENERICDATE4, GENERICDATE5, GENERICBOOLEAN2, 
                                        ...
                                        TEX0_GENERICNUMBER1, TEX0_GENERICNUMBER2, TEX0_GENERICNUMBER3, TEX0_GENERICNUMBER4, TEX0_GENERICNUMBER5,PROCESSINGUNITSEQ, 
										GENERICATTRIBUTE26, GENERICATTRIBUTE27, GENERICATTRIBUTE28, GENERICATTRIBUTE29, GENERICATTRIBUTE32, ALTERNATEORDERNUMBER, GENERICNUMBER4, TEX0_GENERICNUMBER9,
										TEX0_GENERICNUMBER10, GENERICNUMBER5, TEX0_GENERICDATE1, CHANNEL )
    SELECT 
        TXN.TENANTID ,
        iperiodseq,   
        PER.NAME,     
    ...
    FROM cs_salestransaction txn 
		INNER JOIN cs_salesorder ordtxn
			...
			AND txn.tenantid = itenantId
			AND ordtxn.processingunitseq = txn.processingunitseq
			AND txn.compensationdate BETWEEN ADD_MONTHS(v_periodstartdate, -2) AND v_periodenddate
			AND txn.modelseq = 0
			AND txn.processingunitseq = iprocessingUnitSeq
			AND ordtxn.tenantid = txn.tenantid
        
        INNER JOIN CS_PERIOD PER 
        on TXN.compensationdate >= per.startdate and TXN.compensationdate < per.enddate and per.periodtypeseq= 2814749767106561 and per.removedate=TO_DATE ('01/01/2200','DD/MM/YYYY') 
    ...
    w_debug('Fin Carga de la tabla ENEL_TXN_TEMP_CEIS_2: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    BEGIN
        SYS.DBMS_STATS.GATHER_TABLE_STATS (
        OwnName => 'ENELEXT'
        ,TabName => 'ENEL_TXN_TEMP_CEIS_2'
        ,Estimate_Percent => 1
        ,Method_Opt => 'FOR ALL COLUMNS SIZE 1'
        ,Degree => 10
        ,Cascade => TRUE
...
begin
    ...
    w_debug('Inicio Borrado de la tabla ENEL_MEDIDAS_TEMP_CEIS.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_MEDIDAS_TEMP_CEIS WHERE  ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_MEDIDAS_TEMP_CEIS.', v_contador_debug);

    w_debug('Insertando datos en tabla ENEL_MEDIDAS_TEMP_CEIS.' ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_MEDIDAS_TEMP_CEIS (TENANTID,PERIODSEQ, PERIODO, NAME, VALUE, MEASUREMENTSEQ, PAYEESEQ, POSITIONSEQ, PIPELINERUNSEQ, PLANSEQ, RULESEQ)
...
/*      
  ...
  */              
    ...
    w_debug('Fin Carga de la tabla ENEL_MEDIDAS_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_MEDIDAS_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_MEDIDAS_TEMP_CEIS',v_contador_debug);
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_CREDIT_TEMP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CEIS( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
    ...
    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_CREDIT_TEMP_CEIS.',v_contador_debug);
	
	
	w_debug('Inicio Truncado de la tabla ENEL_CREDIT_TEMP_CEIS_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEIS_2';
    w_debug('Fin Truncado de la tabla ENEL_CREDIT_TEMP_CEIS_2.', v_contador_debug);
	v_periodstartdate :=  f_fecha_inicio(iperiodseq);
	v_periodenddate := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_CREDIT_TEMP. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_CREDIT_TEMP_CEIS_2( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, NAME, CREDITSEQ, SALESORDERSEQ, SALESTRANSACTIONSEQ, PAYEESEQ, POSITIONSEQ, COMPENSATIONDATE,  
											COMMENTS, CREDITTYPEID, CREDITTYPEDESCRIPT, VALUE, PREADJUSTEDVALUE, GENERICATTRIBUTE1, GENERICATTRIBUTE2, GENERICATTRIBUTE3, GENERICATTRIBUTE4, 
											GENERICATTRIBUTE5, GENERICATTRIBUTE6, GENERICATTRIBUTE7, GENERICATTRIBUTE8, GENERICATTRIBUTE9, GENERICATTRIBUTE10, GENERICATTRIBUTE11, 
											GENERICATTRIBUTE12, GENERICATTRIBUTE13, GENERICATTRIBUTE14,GENERICATTRIBUTE15, GENERICBOOLEAN1, GENERICBOOLEAN2, GENERICDATE1, 
											GENERICNUMBER1, GENERICNUMBER2, GENERICNUMBER3, GENERICNUMBER4, BU_NAME, GENERICNUMBER6 )
    ...
    FROM CS_CREDIT credit
        ...
        INNER JOIN cs_period per
            on credit.compensationdate >= per.startdate and credit.compensationdate < per.enddate and per.periodtypeseq= 2814749767106561 and per.removedate=TO_DATE ('01/01/2200','DD/MM/YYYY') 
    ...
    WHERE
        credit.TENANTID = itenantId 
        AND credit.PROCESSINGUNITSEQ = iprocessingUnitSeq 
        AND credit.compensationdate BETWEEN ADD_MONTHS(v_periodstartdate, -2) AND v_periodenddate; 
            
    filas := sql%rowcount;
    COMMIT;

    w_debug('Fin Carga de la tabla ENEL_CREDIT_TEMP_CEIS_2: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_CREDIT_TEMP_CEIS_2 COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_CREDIT_TEMP_CEIS_2.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_COMMISSION_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_COMMISSION_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_COMMISSION_TEMP_CEIS. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_COMMISSION_TEMP_CEIS( TENANTID, PERIODSEQ, PERIODO, COMMISSIONSEQ, CREDITSEQ, INCENTIVESEQ, ENTRYNUMBER, PIPELINERUNSEQ, PAYEESEQ, POSITIONSEQ, ORIGINTYPEID, PIPELINERUNDATE, 
    ...
    w_debug('Fin Carga de la tabla ENEL_COMMISSION_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMMISSION_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMMISSION_TEMP_CEIS.',v_contador_debug);
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_INCEN_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_INCEN_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_INCEN_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_INCEN_TEMP_CEIS. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_INCEN_TEMP_CEIS(  TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, INCENTIVESEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE,
    ...
    w_debug('Fin Carga de la tabla ENEL_INCEN_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_INCEN_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_INCEN_TEMP_CEIS.',v_contador_debug);
end;         
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_DEPOSIT_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_DEPOSIT_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_DEPOSIT_TEMP_CEIS. Periodo:'|| iperiod ,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_DEPOSIT_TEMP_CEIS( TENANTID, PERIODSEQ, PERIODO, PIPELINERUNSEQ, PIPELINERUNDATE, DEPOSITSEQ, PAYEESEQ, POSITIONSEQ, NAME, VALUE, 
												PREADJUSTEDVALUE, EARNINGCODEID, EARNINGGROUPID, COMMENTS, GENERICATTRIBUTE1, GENERICATTRIBUTE2, tipo_pago_ga5, 
												processingunitseq, wbe, BUSINESSUNITMAP, LINEA_NEGOCIO, TIPO, BU_NAME, DETALLE_LINEA, GENERICNUMBER1,GENERICDATE1,GENERICATTRIBUTE9 )--DMS 28.06.2024
    SELECT
        depo.TENANTID,
        depo.PERIODSEQ,
        iperiod PERIODO,
		...
		BU.NAME,
        DEPO.GENERICATTRIBUTE8 AS DETALLE_LINEA,
    ...
    w_debug('Fin Carga de la tabla ENEL_DEPOSIT_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_DEPOSIT_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_DEPOSIT_TEMP_CEIS.',v_contador_debug);   
end;         
...
AS
    ...
    v_periodstartdate date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEIS.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_ORDER_IMPU_TEMP_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS( TENANTID,PERIODSEQ,IDPROVEEDOR,DESCRIPCION,DESCRIPCION_CORTA, FICHERO,CECO,
    ...
    w_debug('Fin Carga de la tabla ENEL_ORDER_IMPU_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS.',v_contador_debug); 
	
	
	
	  w_debug('Inicio Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEIS_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS_2';
    w_debug('Fin Truncado de la tabla ENEL_ORDER_IMPU_TEMP_CEIS_2.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);
    v_periodstartdate :=  f_fecha_inicio(iperiodseq);

    w_debug('Cargando tabla ENEL_ORDER_IMPU_TEMP_CEIS_2. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS_2( TENANTID,PERIODSEQ,IDPROVEEDOR,DESCRIPCION,DESCRIPCION_CORTA, FICHERO,CECO,
													WBE_FINAL_IMPUTACION,DETALLE_ACTIVIDAD,ACTIVIDAD,SOCIEDAD,
													CENTRO_LOGISTICO,GR_COMPRAS, ORG_VENTAS, FECHA_INICIO_VIGOR,FECHA_FIN_VIGOR,SUBACTIVIDAD, NOM_SOLICITANTE)
    SELECT 
        itenantId TENANTID,
        PER.PERIODSEQ PERIDOSEQ,
        C.CLASSIFIERID IDPROVEEDOR,
        C.NAME DESCRIPCION,
        GC.GENERICATTRIBUTE8 DESCRIPCION_CORTA,
        C.DESCRIPTION FICHERO,
    ...
    FROM CS_GENERICCLASSIFIERTYPE GCT
        INNER JOIN CS_CLASSIFIER C 
            ...
            AND C.REMOVEDATE = v_eot
            AND ((C.EFFECTIVESTARTDATE <= v_ultimo_dia_periodo
            AND C.EFFECTIVEENDDATE >= v_ultimo_dia_periodo) or 
			(C.EFFECTIVESTARTDATE <= ADD_MONTHS(v_ultimo_dia_periodo, -1)
            AND C.EFFECTIVEENDDATE >= ADD_MONTHS(v_ultimo_dia_periodo, -1)) or 
			(C.EFFECTIVESTARTDATE <= ADD_MONTHS(v_ultimo_dia_periodo, -2)
            AND C.EFFECTIVEENDDATE >= ADD_MONTHS(v_ultimo_dia_periodo, -2)))
            -- AND C.ISLAST = 1
            
        INNER JOIN CS_PERIOD PER
            ON (PER.STARTDATE = v_periodstartdate
            OR PER.STARTDATE = ADD_MONTHS(v_periodstartdate, -1)
            OR PER.STARTDATE = ADD_MONTHS(v_periodstartdate, -2))
        ...
        INNER JOIN CS_GENERICCLASSIFIER GC 
            ...
            AND GC.REMOVEDATE = v_eot
            AND ((GC.EFFECTIVESTARTDATE <= v_ultimo_dia_periodo
            AND GC.EFFECTIVEENDDATE >= v_ultimo_dia_periodo) or 
			(GC.EFFECTIVESTARTDATE <= ADD_MONTHS(v_ultimo_dia_periodo, -1)
            AND GC.EFFECTIVEENDDATE >= ADD_MONTHS(v_ultimo_dia_periodo, -1)) or 
			(GC.EFFECTIVESTARTDATE <= ADD_MONTHS(v_ultimo_dia_periodo, -2)
            AND GC.EFFECTIVEENDDATE >= ADD_MONTHS(v_ultimo_dia_periodo, -2)))
            -- AND GC.ISLAST = 1
    ...
    w_debug('Fin Carga de la tabla ENEL_ORDER_IMPU_TEMP_CEIS_2: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS_2 COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS_2.',v_contador_debug); 
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_EQUIPAMIENTO_TEMP_CEIS.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_EQUIPAMIENTO_TEMP_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);

    INSERT INTO ENELEXT.ENEL_EQUIPAMIENTO_TEMP_CEIS( TENANTID,PERIODSEQ,EQUIPAMIENTO, IDMARCA, MARCA, IDMODELO, MODELO, IDTIPO,
    ...
    w_debug('Fin Carga de la tabla ENEL_EQUIPAMIENTO_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEIS.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CEIS WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Truncado de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEIS.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Origen de ENEL_E4E_NEGATIVOS_TEMP_CEIS : CS_DEPOSIT.', v_contador_debug);
    ...
    INSERT INTO ENELEXT.ENEL_E4E_NEGATIVOS_TEMP_CEIS( PERIODSEQ,PERIODO, DEPOSITSEQ,POSITIONSEQ,PAYEESEQ,VALUE,PDS,
    ...
    FROM ENEL_DEPOSIT_TEMP_CEIS DEPO
        ...
        LEFT JOIN ENEL_E4E_CONTRATOS_TEMP_CEIS TMP_CONTRA
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_NEGATIVOS_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
end;
...
AS
    v_ultimo_dia_periodo  date;
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_E4E_CONTRATOS_TEMP_CEIS.', v_contador_debug);

    v_ultimo_dia_periodo := f_Ultimo_Dia_Periodo(iperiodseq);

    w_debug('Cargando tabla ENEL_E4E_CONTRATOS_TEMP_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId || ' Ult.Dia Period:' || v_ultimo_dia_periodo,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEIS(TENANTID,PERIODSEQ,ID,PDS,ACTIVIDAD_DETALLADA,ACTIVIDAD, CIF,COD_CONTRATO,POS_DOC,TEXTO_BREVE,CODIGO_SERVICIO,
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_CONTRATOS_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEIS.',v_contador_debug);   
end;
...
begin
    ...
    w_debug('Inicio Truncado de la tabla ENEL_PDS_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_PDS_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_PDS_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_PDS_TEMP_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_PDS_TEMP_CEIS( PERIODSEQ, RULEELEMENTOWNERSEQ, PAYEESEQ, PAYEEID, PDS, NOMBRE_FISCAL, CIF, NOMBRE_CUENTA, CALLE, COD_POSTAL, 
    ...
    w_debug('Fin Carga de la tabla ENEL_PDS_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PDS_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PDS_TEMP_CEIS.',v_contador_debug);
	
	
	
	 w_debug('Inicio Truncado de la tabla ENEL_PDS_TEMP_CEIS_2.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_PDS_TEMP_CEIS_2';
    w_debug('Fin Truncado de la tabla ENEL_PDS_TEMP_CEIS_2.', v_contador_debug);
	v_periodstartdate :=  f_fecha_inicio(iperiodseq);

    w_debug('Cargando tabla ENEL_PDS_TEMP_CEIS_2. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);
      
    INSERT INTO ENELEXT.ENEL_PDS_TEMP_CEIS_2( PERIODSEQ, RULEELEMENTOWNERSEQ, PAYEESEQ, PAYEEID, PDS, NOMBRE_FISCAL, CIF, NOMBRE_CUENTA, CALLE, COD_POSTAL, 
                                          PROVINCIA, POBLACION, TIPO_IMPOSITIVO, PAR_PROVEEDOR, CODIGODEUDOR, NOMBRE_COMERCIAL, IMPORTE_UB, FECHA_CONTRATACION, 
                                          TIPO_PRESTADOR, COMUNIDAD_AUTONOMA, TERRITORIO, ZONA, POS_NOMBRE_COMERCIAL, CANAL, SUBCANAL, DELEGACION, 
                                          BASE_COMISION, FECHA_INI_VIGENCIA,TERMINATIONDATE,TITLE_NAME,CANAL_CALCULOS,TIPO_POSICION, NOMBRE_FISCAL_CORTO )
    SELECT 
    ...
    WHERE
        per.REMOVEDATE = v_eot
        AND (per.startdate = v_periodstartdate
            OR per.startdate = ADD_MONTHS(v_periodstartdate, -1)
            OR per.startdate = ADD_MONTHS(v_periodstartdate, -2))
		;
        --and par.GENERICATTRIBUTE1 is not null; -- MPR esta condicion no aplica a CES
    ...
    w_debug('Fin Carga de la tabla ENEL_PDS_TEMP_CEIS_2: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PDS_TEMP_CEIS_2 COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PDS_TEMP_CEIS_2.',v_contador_debug);
end;
...
AS    
begin
    w_debug('Inicio Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CEIS.', v_contador_debug);
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEIS';
    w_debug('Fin Truncado de la tabla ENEL_E4E_DEPOSIT_TEMP_CEIS.', v_contador_debug);

    w_debug('Cargando tabla ENEL_E4E_DEPOSIT_TEMP_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    -- Si se ejecuta en una FASE que no es REWARD Utilizamos la tabla de PAGOS
    w_debug('Origen de ENEL_E4E_DEPOSIT_TEMP_CEIS.', v_contador_debug);

	INSERT INTO ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEIS( PERIODSEQ, PERIODO, POSITIONSEQ,PAYEESEQ,VALUE,PDS,PAR_PROVEEDOR,TIPO_IMPOSITIVO,COD_CONTRATO,POS_DOC,
    ...
    FROM  ENELEXT.enel_deposit_temp_CEIS DEPO				 
		INNER JOIN ENELEXT.ENEL_PDS_TEMP_CEIS TMP_PDS 
			ON depo.payeeseq=TMP_PDS.payeeseq 
			and depo.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
			and depo.periodseq=TMP_PDS.periodseq

		INNER JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS TMP_PROV 
			ON TMP_PROV.IDPROVEEDOR=depo.earninggroupid  
			AND depo.periodseq=TMP_PROV.periodseq

		LEFT JOIN ENELEXT.ENEL_E4E_CONTRATOS_TEMP_CEIS TMP_CONTRA
			ON TMP_CONTRA.periodseq=depo.periodseq
			AND TMP_CONTRA.PDS = TMP_PDS.PDS
			AND TMP_CONTRA.ACTIVIDAD_DETALLADA=TMP_PROV.DETALLE_ACTIVIDAD
			AND TMP_CONTRA.periodseq=TMP_PROV.periodseq
    ...
    w_debug('Fin Carga de la tabla ENEL_E4E_DEPOSIT_TEMP_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_E4E_DEPOSIT_TEMP_CEIS.',v_contador_debug);
end;
...
AS
    contador integer;  
begin
    w_debug('Inicio Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CEIS para fichero ' || iFichero ,  v_contador_debug);
    contador := 1;
    -- Registro de CABECERA 1 : lista de campos
    INSERT INTO ENEL_E4E_FINAL_CEIS (PERIODO, ORDEN, CAMPO1, CAMPO2, CAMPO3, CAMPO4, CAMPO5, CAMPO6, CAMPO7, CAMPO8, CAMPO9, CAMPO10, 
    ...
    COMMIT;
    w_debug('Fin Inserccion 4 Registros fijos de cabecera en tabla ENEL_E4E_FINAL_CEIS para fichero ' || iFichero ,  v_contador_debug);
...
AS
	...
	v_aux VARCHAR(10);
begin
    w_debug('Inicio Borrado de la tabla ENEL_E4E_FINAL_CEIS.', v_contador_debug);
   
    --v_codFichero :='E4E1';  -- v2.0 se asigna el valor dinamicamente en funcion de la actividad del proveedor
        
	-- EXECUTE IMMEDIATE 'DELETE ENELEXT.ENEL_E4E_FINAL_CEIS WHERE ....';
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CEIS WHERE PERIODO = iperiod AND FICHERO like 'E%1' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
    w_debug('Fin Borrado de la tabla ENEL_E4E_FINAL_CEIS.', v_contador_debug);
    ...
    BEGIN
		LOOP
			DELETE FROM ENEL_AGR_FAC_E4E_FINAL_CES WHERE PERIODO = iperiod AND CANAL = 'IS' AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;    
    ...
    w_debug('Cargando tabla ENEL_E4E_FINAL_CEIS. Periodo:'|| iperiod ||' Periodseq: '||iperiodseq ||' TenantId: '||itenantId ,  v_contador_debug);

    w_debug('Insertando Registros de datos en tabla ENEL_E4E_FINAL_CEIS. Fichero ' || v_codFichero ,  v_contador_debug);
    ...
    DECLARE
        CURSOR C_TMPDEPOSITOS IS
            ...
            FROM ENEL_E4E_DEPOSIT_TEMP_CEIS
            ...
            IF REGDEPOSITO.TIPO_PAGO = 'SERVICIO' THEN
				...
				VALUES ( iperiod,
                        ...
                        VALUES ( iperiod,
                                ...
                                v_referencia,
                                'IS',
                                SUBSTR(REGDEPOSITO.DESCRIPCION, 1, INSTR(REGDEPOSITO.DESCRIPCION, ' ') )
                        );*/
                        -- REQ-CM-003: Informe de Balance para Clientes Empresa - fin - 05.11.2021
            END IF;
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CEIS:  E4E1 '|| to_char(contadorE4E) || ' -- ECS1 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
    ...
    INSERT INTO ENELEXT.ENEL_AGR_FAC_E4E_FINAL_CES  (PERIODO, PDS, IMPORTE_NUM, DET_ORDEN, CANAL, LINEA_NEGOCIO)
        SELECT 
                iperiod as periodo,
                pds,
                value as IMPORTE_NUM,
                PDS || SUBSTR(LINEA_NEGOCIO,1,1) || SUBSTR(CAMPANIA,1,1) || TIPO || v_mes || v_txtYear as det_orden,
                'IS',
                linea_negocio
        FROM ENEL_E4E_DEPOSIT_TEMP_CEIS
        WHERE PERIODSEQ = iperiodseq
...
begin
	...
	BEGIN
		LOOP
			DELETE FROM ENEL_E4E_FINAL_CEIS WHERE PERIODO = iperiod AND FICHERO like 'E%2'  AND ROWNUM <= 10000;
			EXIT WHEN SQL%ROWCOUNT = 0;
			COMMIT;
		END LOOP;
	END;
	...
	w_debug('Fin Carga de la tabla ENEL_E4E_FINAL_CEIS:  E4E2 '|| to_char(contadorE4E) || ' -- ECS2 '|| to_char(contadorECS) || ' filas.', v_contador_debug);
...
begin
    ...
    FROM ENEL_E4E_NEGATIVOS_TEMP_CEIS e4edt 
...
begin
    w_debug('Inicio Borrado de la tabla ENEL_LIQUIDACION_CEIS.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_LIQUIDACION_CEIS WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_LIQUIDACION_CEIS.', v_contador_debug);
    ...
    w_debug('Insertando CREDITOS de datos en tabla ENEL_LIQUIDACION_CEIS.' ,  v_contador_debug);
    -- v2.0 Se cambia la tabla de origen CS_CREDIT  a la temporal ENEL_CREDIT_TEMP
    INSERT INTO ENELEXT.ENEL_LIQUIDACION_CEIS ( PERIODO, PROVEEDOR, ANO_LIQUIDACION, MES_LIQUIDACION, CODIGO_AGENTE_INTERNO, CONCEPTO, CANTIDAD, 
    ...
    FROM ENEL_CREDIT_TEMP_CEIS credtmp
        INNER JOIN ENEL_PDS_TEMP_CEIS TMP_PDS 
            ON credtmp.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ
		LEFT JOIN ENELEXT.ENEL_ORDER_IMPU_TEMP_CEIS TMP_PROV
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_LIQUIDACION_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando INCENTIVOS de datos en tabla ENEL_LIQUIDACION_CEIS.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CEIS INCENTMP    
        INNER JOIN ENEL_PDS_TEMP_CEIS TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_LIQUIDACION_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_LIQUIDACION_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_LIQUIDACION_CEIS.',v_contador_debug);
end;
...
begin
    ...
    INSERT INTO ENELEXT.ENEL_CUADRELIQ_FINAL_CES (PERIODO, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTTYPEID, IMPORTE, UNIDAD, VALOR_1, UNIDAD_1, PRODUCTO_SCAWEB, 
													...
													CUPS, CIF, NOMBRE_CLIENTE, SEGMENTO, PROVINCIA, TERRITORIO, CONSUMO_TXN, COMPONENTE, PAYEESEQ, POSITIONSEQ, CONDICION_MULTIPUNTO,DURACION,
                                                    CONTRATO, --APM 05.06.2023 Nuevo campo
    ...
    SELECT 
        ...
        --BOM DMS 27.04.2023 CAMBIO COMM POR CRED
		case 
			when ECT.GENERICATTRIBUTE2 = 'ELECTRICIDAD' and ince.name like 'C - CE - IS - Electricidad - Importe Base%' then TRIM(to_char(ECT.VALUE , '9999999999990D99')) 
			when ECT.GENERICATTRIBUTE2 = 'GAS' and ince.name  like 'C - CE - IS - Gas - Importe Base%' then TRIM(to_char(ECT.VALUE , '9999999999990D99')) 
			when (ECT.GENERICATTRIBUTE2 not like 'ELECTRICIDAD' or ECT.GENERICATTRIBUTE2 not like 'GAS') then TRIM(to_char(ECT.VALUE , '9999999999990D99')) 
			else '0'
		end as IMPORTE_COMISION,
        --EOM DMS 27.04.2023 CAMBIO COMM POR CRED
        'EURO',
        ...
        ETT.GENERICNUMBER1 as DURACION, --DIEGO   AÑADIR CAMPO  10/11/22
        ETT.PONUMBER AS CONTRATO, --APM 05.06.2023
    ...
    FROM ENEL_TXN_TEMP_CEIS ETT
        LEFT JOIN ENEL_CREDIT_TEMP_CEIS ECT
            ON ECT.SALESTRANSACTIONSEQ = ETT.SALESTRANSACTIONSEQ
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CEIS TEMP_PROV
            ON TEMP_PROV.IDPROVEEDOR=ECT.GENERICATTRIBUTE4
		LEFT JOIN ENEL_PDS_TEMP_CEIS TMP_PDS 
			ON ECT.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ 
		
		LEFT JOIN ENEL_COMMISSION_TEMP_CEIS COMMI
			ON COMMI.CREDITSEQ = ECT.CREDITSEQ
			AND COMMI.PAYEESEQ = ECT.PAYEESEQ
		LEFT JOIN ENEL_INCEN_TEMP_CEIS INCE
    ...
    v_fechaInicioPeriodo :=  f_fecha_inicio(iperiodseq);
    v_anio := to_char(f_fecha_inicio(iperiodseq),'YYYY');
    ...
    INSERT INTO ENELEXT.ENEL_DET_INCENTIVOS_CES (PERIODO, ORDERID, LINENUMBER, SUBLINENUMBER, EVENTTYPEID, IMPORTE, UNIDAD, VALOR_1, UNIDAD_1, PRODUCTO_SCAWEB, 
													...
													ESTADO_CTRLCALIDAD, Nom_Credito, PRODUCTO, CAMPANIA, TARIFA, CONSUMO, DESCUENTO, POTENCIA, PVP,
													NOMBRE_FISCAL, LINEA_NEGOCIO, FECHA_INSERCION, FECHA_FIRMA, PROCESSINGUNITSEQ, CANAL, OFERTA, AGENTE, COMENTARIOS, ANIO, 
													CUPS, CIF, NOMBRE_CLIENTE, SEGMENTO, PROVINCIA, TERRITORIO, CONSUMO_TXN, COMPONENTE, PAYEESEQ, POSITIONSEQ,DURACION,
                                                    CONTRATO) --APM 05.06.2023 Nuevo campo
    SELECT 
        /*CASE 
        WHEN ince.name = 'C - CE - IS - Electricidad - Incentivo Objetivos (ud) Trim NN 0 (Capt)' 
             OR ince.name = 'C - CE - IS - Electricidad - Incentivo Objetivos (ud) Trim NN -1 (Capt)'
             OR ince.name = 'C - CE - IS - Electricidad - Incentivo Objetivos (ud) Trim NN -2 (Capt)' 
        THEN 
            CASE 
                WHEN ETT.PERIODO LIKE 'Febrero%' OR ETT.PERIODO LIKE 'Marzo%' OR ETT.PERIODO LIKE 'Abril%'
                THEN 'Abril ' || v_anio
                WHEN ETT.PERIODO LIKE 'Mayo%' OR ETT.PERIODO LIKE 'Junio%' OR ETT.PERIODO LIKE 'Julio%'
                THEN 'Julio ' || v_anio
                WHEN ETT.PERIODO LIKE 'Agosto%' OR ETT.PERIODO LIKE 'Septiembre%' OR ETT.PERIODO LIKE 'Octubre%'
                THEN 'Octubre ' || v_anio
                WHEN ETT.PERIODO LIKE 'Noviembre%' OR ETT.PERIODO LIKE 'Diciembre%' 
				THEN 'Enero ' || TO_CHAR(TO_NUMBER(v_anio) + 1)
				WHEN ETT.PERIODO LIKE 'Enero%'
                THEN 'Enero ' || v_anio
            END
		END AS PERIODO,*/
        ETT.PERIODO,
    ...
    FROM ENEL_TXN_TEMP_CEIS_2 ETT
        LEFT JOIN ENEL_CREDIT_TEMP_CEIS_2 ECT
            ON ECT.SALESTRANSACTIONSEQ = ETT.SALESTRANSACTIONSEQ
        LEFT JOIN ENEL_ORDER_IMPU_TEMP_CEIS_2 TEMP_PROV
            ON TEMP_PROV.IDPROVEEDOR=ECT.GENERICATTRIBUTE4
            and TEMP_PROV.periodseq = ect.periodseq
		INNER JOIN ENEL_PDS_TEMP_CEIS_2 TMP_PDS 
			ON ECT.POSITIONSEQ=TMP_PDS.RULEELEMENTOWNERSEQ 
            and TMP_PDS.periodseq =ect.periodseq
		...
		/*INNER JOIN cs_commission
			on commi.CREDITSEQ = ECT.creditseq
            and commi.payeeseq = ECT.payeeseq*/
    
	WHERE (ince.name not like 'C - CE - IS - Electricidad - Importe Base%'
		and ince.name not like 'C - CE - IS - Gas - Importe Base%')
...
begin
    ...
    /*BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES WHERE PERIODO = iperiod AND CANAL = 'IS' AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;*/
    DELETE FROM ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES WHERE PERIODO = iperiod AND CANAL = 'IS';
    ...
    INSERT INTO ENELEXT.ENEL_REMUN_CES (PERIODO,NAME,VALUE,PAYEEID, PROCESSINGUNITSEQ, NOMBRE_FISCAL, CANAL, ANIO, PAYEESEQ, POSITIONSEQ, NUMERO_RESUMEN, CIF, PDS, 
										FECHA_LIQUIDACION, IVA, LINEA_NEGOCIO, CONCEPTO_LIQ, IDPROVEEDOR, TIPO_PRESTADOR, OBSERVACIONES)
    Select 
        EDT.periodo,
        EDT.name,
		...
		CSP.TIPO_PRESTADOR,
		EDT.GENERICATTRIBUTE4
		
    from ENELEXT.ENEL_INCEN_TEMP_CEIS edt
		INNER JOIN ENEL_PDS_TEMP_CEIS CSP
...
--		and value > 0; 7feb22 Se solicita que aparezcan negativos en Cuadre Liq/Total Precio Base Fijo
    ...
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - inicio - 05.11.2021
    w_debug('Insertando Registros de datos en tabla ENEL_AGR_FAC_PREFACTURA_CES.' ,  v_contador_debug);
    ...
    FROM (  SELECT
                EDT.periodo,
                EDT.BU_NAME || v_mesanio || CSP.PDS || SUBSTR(EDT.GENERICATTRIBUTE2,0,1) || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) as cod_prefactura,
                EDT.VALUE,
                CSP.PDS || 
                  CASE
                    WHEN EDT.GENERICATTRIBUTE4 LIKE 'Ratios Actividad Comercial%' THEN 'R' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE 'Posici%n Activa%' THEN 'P' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Reporting de Actividad' THEN 'R' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Program Management' THEN 'P' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Cumplimiento Modelo Operativo' THEN 'C' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Integraci%n Plataforma y Set-Up' THEN 'I' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PF'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Activaci%n Canales Digitales' THEN 'A' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'RVAC'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Mantenimiento CT%' THEN 'M' || SUBSTR(EDT.GENERICATTRIBUTE6,0,1) || 'PB'
                  END
                  /*CASE
                    WHEN EDT.GENERICATTRIBUTE6 LIKE 'CAPTACI%N' THEN 'PB'
                    WHEN EDT.GENERICATTRIBUTE6 LIKE 'RENOVACI%N' THEN 'PF'
                  END*/
                || v_mesanio as det_orden,
                 
               CASE
                    WHEN EDT.GENERICATTRIBUTE4 LIKE 'Ratios Actividad Comercial%' THEN 'Ratios Actividad Comercial'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE 'Posici%n Activa%' THEN 'Posición Activa'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Reporting de Actividad' THEN 'Reporting de Actividad'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Program Management' THEN 'Program Management'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Cumplimiento Modelo Operativo' THEN 'Cumplimiento Modelo Operativo'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Integraci%n Plataforma y Set-Up' THEN 'Integración Plataforma y Set-Up'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Activaci%n Canales Digitales' THEN 'Activación Canales Digitales'
                    WHEN EDT.GENERICATTRIBUTE4 LIKE '%Mantenimiento CT%' THEN 'Mantenimiento CT'
            ...
            WHERE (name  like 'I - CE - % - Precio Base %')
                AND value > 0
                AND EDT.BU_NAME = 'CEIS'
                AND ( EDT.GENERICATTRIBUTE6 LIKE 'CAPTACI%N' OR EDT.GENERICATTRIBUTE6 LIKE 'RENOVACI%N')
        )

    GROUP BY periodo, cod_prefactura, det_orden, LINEA_NEGOCIO
    ORDER BY det_orden;
	
	filas := sql%rowcount;
	COMMIT;
    w_debug('Fin Carga de la tabla ENEL_AGR_FAC_PREFACTURA_CES: '|| to_char(filas) || ' filas.', v_contador_debug);
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - fin - 05.11.2021
...
begin
    ...
    INSERT INTO ENELEXT.ENEL_RAPPELES_CES(PERIODO, NAME, IMPORTE, CODIGO_PDS_OCAP, CICLO_FACTURACION, IDPROVEEDOR, CONCEPTO, TRAMO, NOMBRE_FISCAL, PROCESSINGUNITSEQ, 
											CANAL, ANIO, REALIZADO, OBJETIVO, CONSECUCION, IMPORTE_UNITARIO, LINEA_NEGOCIO, CAMPANIA, PAYEESEQ, POSITIONSEQ)     
    SELECT 
        iperiod,
        CSI.NAME,
    ...
    FROM ENELEXT.ENEL_INCEN_TEMP_CEIS CSI
       	INNER JOIN ENEL_PDS_TEMP_CEIS TMP_PDS
    ...
    WHERE 
		...
		and csi.name like '%Incentivo Objetivos%';
...
AS
    ...
    v_mesanio VARCHAR(4);
begin
    w_debug('Inicio Borrado de la tabla ENEL_PREFACTURA_CEIS.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_PREFACTURA_CEIS WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_PREFACTURA_CEIS.', v_contador_debug);
    ...
    w_debug('Insertando Registros de CREDITOS en tabla ENEL_PREFACTURA_CEIS.' ,  v_contador_debug);
   
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CEIS ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, DESCRIPCION,
    ...
    SELECT 
        ...
        --BOM DMS 27.04.2023 CAMBIO COMM POR CRED
		case 
			when CREDTMP.GENERICATTRIBUTE2 = 'ELECTRICIDAD' and ince.name like 'C - CE - IS - Electricidad - Importe Base%' then TRIM(to_char(credtmp.VALUE , '9999999999990D99')) 
			when CREDTMP.GENERICATTRIBUTE2 = 'GAS' and ince.name  like 'C - CE - IS - Gas - Importe Base%' then TRIM(to_char(credtmp.VALUE , '9999999999990D99')) 
			when (CREDTMP.GENERICATTRIBUTE2 not like 'ELECTRICIDAD' or CREDTMP.GENERICATTRIBUTE2 not like 'GAS') then TRIM(to_char(CREDTMP.VALUE , '9999999999990D99')) 
			else '0'
		end as value,
        --EOM DMS 27.04.2023 CAMBIO COMM POR CRED
    ...
    FROM ENEL_CREDIT_TEMP_CEIS CREDTMP
        INNER JOIN ENEL_TXN_TEMP_CEIS TXNTMP 
    ...
    w_debug('Fin Carga CREDITOS de la tabla ENEL_PREFACTURA_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
    

    w_debug('Insertando Registros de INCENTIVOS en tabla ENEL_PREFACTURA_CEIS.' ,  v_contador_debug);
    ...
    FROM ENEL_INCEN_TEMP_CEIS INCETMP
        INNER JOIN ENEL_PDS_TEMP_CEIS TMP_PDS
    ...
    w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - inicio - 05.11.2021
    -- PDS = PDS + LINEA_NEGOCIO +  CAMPANIA + Tipo? + MMYY
    w_debug('Insertando Registros PREFACTURA en tabla ENEL_AGR_FAC_PREFACTURA_CES.' ,  v_contador_debug);
    INSERT INTO ENELEXT.ENEL_AGR_FAC_PREFACTURA_CES ( PERIODO, PDS, IMPORTE, DET_ORDEN, CANAL, LINEA_NEGOCIO)
    SELECT periodo, 
           mypds, 
           sum(value), 
           ...
           'IS',
           CASE 
                WHEN tprefact.LINEA_NEGOCIO LIKE 'BATERIA CONDENSADORES' THEN 'BBCC'
                ELSE to_char(tprefact.LINEA_NEGOCIO)
           END LINEA_NEGOCIO
    FROM (SELECT    periodo, 
                    periodseq,
                    value,
                    linea_negocio, 
                    tipo, 
                    numero_resumen || SUBSTR(linea_negocio,1,1) || SUBSTR(concepto_liq,1,1) as mypds,
                    pds || SUBSTR(linea_negocio,1,1) || SUBSTR(concepto_liq,1,1) || 'PB' || v_mesanio as myDET_ORDEN 
          FROM ENELEXT.ENEL_PREFACTURA_CEIS pref) tprefact
    WHERE periodseq = iperiodseq
            --AND UPPER(LINEA_NEGOCIO) = 'ELECTRICIDAD'
            --AND UPPER(TIPO) = 'CREDIT'
    GROUP BY periodo, mypds, mydet_orden, LINEA_NEGOCIO;
        
    filas := sql%rowcount;
    COMMIT;
    w_debug('Fin Carga PREFACTURAS de la tabla ENEL_AGR_FAC_PREFACTURA_CES: '|| to_char(filas) || ' filas.', v_contador_debug);
    -- REQ-CM-003: Informe de Balance para Clientes Empresa - fin - 05.11.2021
	
/* ==>> Solo para TVTA, resto de canales comentado
	w_debug('Insertando Registros de REMUN, precio BASE en tabla ENEL_PREFACTURA_CEIS.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_PREFACTURA_CEIS ( PERIODO, PERIODSEQ, MES_LIQUIDACION, FECHA_LIQUIDACION, PDS, NUMERO_RESUMEN, NOMBRE_FISCAL, CIF, IDPROVEEDOR, 
	...
	w_debug('Fin Carga INCENTIVOS de la tabla ENEL_PREFACTURA_CEIS para Precio BASE: '|| to_char(filas) || ' filas.', v_contador_debug);
*/
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_PREFACTURA_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_PREFACTURA_CEIS.',v_contador_debug);
end;
...
AS
begin
    w_debug('Inicio Borrado de la tabla ENEL_COMP_E4E_CEIS.', v_contador_debug);
    BEGIN
        LOOP
            DELETE FROM ENELEXT.ENEL_COMP_E4E_CEIS WHERE PERIODO = iperiod AND ROWNUM <= 10000;
            EXIT WHEN SQL%ROWCOUNT = 0;
            COMMIT;
        END LOOP;
    END;
    w_debug('Fin Borrado de la tabla ENEL_COMP_E4E_CEIS.', v_contador_debug);

    w_debug('Insertando Registros SCAWEB-E4E de datos en tabla ENEL_COMP_E4E_CEIS.' ,  v_contador_debug);
    
    INSERT INTO ENELEXT.ENEL_COMP_E4E_CEIS ( PERIODO, IDPROVEEDOR, DESCRIPCION, ACTIVIDAD, PDS, NOMBRE_FISCAL, IMPORTE_SCAWEB, E4E_POS_CON_CONTRATO, 
    ...
    FROM
        ( select PERIODO, TRIM(to_char(PROVEEDOR,'000')) IDPROVEEDOR, SCA.CODIGO_AGENTE_INTERNO as PDS, sum(REALVALUE) as IMPORTE_SCAWEB, count(*) registros
            from ENEL_LIQUIDACION_CEIS sca where PERIODO = IPERIOD
    ...
    w_debug('Fin Carga Registros SCAWEB-E4E de la tabla ENEL_COMP_E4E_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);

    w_debug('Insertando Registros E4E-SCAWEB (scaweb nulos) de datos en tabla ENEL_COMP_E4E_CEIS.' ,  v_contador_debug);
    ...
    FROM
        ...
        LEFT JOIN
            ( SELECT PERIODO
                ...
                , COUNT(*) REGISTROS        
                FROM ENEL_LIQUIDACION_CEIS SCA             
    ...
    w_debug('Fin Carga Registros E4E-SCAWEB (scaweb nulos) de la tabla ENEL_COMP_E4E_CEIS: '|| to_char(filas) || ' filas.', v_contador_debug);
    
    EXECUTE IMMEDIATE 'ANALYZE TABLE ENELEXT.ENEL_COMP_E4E_CEIS COMPUTE STATISTICS FOR ALL INDEXES';
    w_debug('Fin Actualizacion Indices ENELEXT.ENEL_COMP_E4E_CEIS.',v_contador_debug);
end; 
...
begin
    ...
    INSERT INTO ENELEXT.ENEL_AGREFACT_FINAL_CES (PERIODO, CANAL, PDS, LINEA_NEGOCIO, CONCEPTO_RETRIBUTIVO, TIPO_GASTO, VALUE, CECO, WBE_FINAL_IMPUTACION, COD_CONTRATO, DEPOSITSEQ, 
												...
												FECHA_PREFACTURA, IMPORTE_CONTRATO_INI, RESTANTE, DET_ORDEN, CONCEPTO_DETALLE, DETALLE_LINEA, CONSUMO_DEPOSITO)
    SELECT 
        ...
        case	
			...
			when DEPOTMP.TIPO = 'DC' or DEPOTMP.name like '%Decomi%' then 'DC'
			else DEPOTMP.TIPO
...
-- Los facturables de CEIS están en comisiones para gas y electricidad y psvas en CDs
-- y no facturables por decirlo asi en cds
...
-- DCR EOM 29.12.2021 INC000079731266 
	...
	WHERE 
		CREDTMP.GENERICATTRIBUTE1 IS NOT NULL 
		AND CREDTMP.VALUE > 0 -- DCR BOM 29.12.2021 INC000079731266 
		AND CREDTMP.NAME NOT LIKE '%Incent%' -- DCR BOM 29.12.2021 INC000079731266 
        --AND COMMI.VALUE > 0 -- DCR BOM 29.12.2021 INC000079731266 
    ...
    SELECT 
		iperiod,
        TMP_PDS.CANAL,  				--CANAL
        COUNT(DEPOTMP.DEPOSITSEQ) AS TOTAL,
        TMP_PDS.PDS AS COD_COLABORADOR,	--CODIGO COLABORADOR / PDS
        UPPER(DEPOTMP.LINEA_NEGOCIO) AS LINEA_NEGOCIO,
        DEPOTMP.GENERICATTRIBUTE1 as TIPO_GASTO,
        DEPOTMP.TIPO as CONCEPTO_RETRIBUTIVO,
        iprocessingUnitSeq
    ...
    WHERE (UPPER(DEPOTMP.LINEA_NEGOCIO) <> 'ELECTRICIDAD' AND
           UPPER(DEPOTMP.LINEA_NEGOCIO) <> 'GAS')

    GROUP BY iperiod,
        TMP_PDS.CANAL,
        TMP_PDS.PDS,
        DEPOTMP.LINEA_NEGOCIO,
        DEPOTMP.GENERICATTRIBUTE1,
        DEPOTMP.TIPO
    ;
	
	w_debug('Fin carga datos en tabla ENEL_REGISTROS_CRED_CES para depositos.' || to_char(filas) || ' filas.', v_contador_debug);
...
-- DCR EOM 29.12.2021 INC000079731266 
	...
	FROM ENEL_CREDIT_TEMP_CEIS CREDTMP
		...
		INNER JOIN ENEL_COMMISSION_TEMP_CEIS COMMI
			ON COMMI.CREDITSEQ = CREDTMP.CREDITSEQ
	...
	SELECT 
		...
		CASE 
			...
			WHEN DIS.REASONCODE LIKE 'Incentivo%' THEN TRIM(REPLACE(DIS.REASONCODE,'Incentivo','')) 
			ELSE DIS.REASONCODE
		END AS TIPO,
		POS.NAME AS PDS,
		iprocessingUnitSeq,
...
BEGIN    
    ...
    if  triggerFilename = 'EJECUCION_MANUAL' then
       ...
       /*BOM APM 14.12.2022 */
       w_debug('Dentro del if - Periodo  '||v_periodo, v_contador_debug);
       /*EOM APM 14.12.2022 */
            ...
            /*EOM APM 12.12.2022 */
			;
			 w_debug('first update'||v_periodo, v_contador_debug);
			--RAPPELES
			...
			and PROCESSINGUNITSEQ = processingunitseq
			;
            w_debug('second update'||v_periodo, v_contador_debug);
			    -- MRMM 12/11/21 BOF
			...
			and PROCESSINGUNITSEQ = processingunitseq
			;
            w_debug('third update'||v_periodo, v_contador_debug);
            --PREFACTURA CEBP
            UPDATE ENELEXT.ENEL_PREFACTURA_CEIS
			SET FECHA_LIQUIDACION = SYSDATE
			WHERE Periodo = period
			and PROCESSINGUNITSEQ = processingunitseq
            ;
             -- MRMM 12/11/21 EOF
             w_debug('fourth update'||v_periodo, v_contador_debug);
            COMMIT; --APM 14.12.2022 
             
        end if;
        
       /*BOM APM 14.12.2022 */
       w_debug('Fuera del if - Periodo  '||v_periodo, v_contador_debug);
       /*EOM APM 14.12.2022 */
    ...
    IF  v_PeriodoLiquidado = false THEN
		...
		if(processingUnit='B2B CE IS') then
            ...
            IF f_ExisteInformeEnLista('E4E_IS', v_Listado_Informes) THEN
			...
			---------------------------------------
			IF f_ExisteInformeEnLista('LIQ_IS', v_Listado_Informes) THEN
				...
				-- Actualizamos la fecha del informes en la tabla 
               	p_Actualiza_Informe_Fecha ( period, 'LIQ_IS');   
            ...
            IF f_ExisteInformeEnLista('E4E_IS', v_Listado_Informes) THEN
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'E4E_IS');
				...
				p_Actualiza_Informe_Fecha ( period, 'E4ENEG_IS');
            end if;

            ----------------------------
            -- Datos para Liquidacion --
            ----------------------------
            IF f_ExisteInformeEnLista('LIQFINAL_IS', v_Listado_Informes) THEN  
                ...
                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'LIQFINAL_IS');
            end if;
            ...
            IF v_Listado_Informes = 'ALL' THEN
				...
				-- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'CANAL_IS');
            end if;        
     
            ---------------------------------------------------
            -- Datos para PREFACTURA
            ---------------------------------------------------
            IF f_ExisteInformeEnLista('PREFACT_IS', v_Listado_Informes) THEN
                -- Extraer datos para detalle de PreFactura
                p_Prefactura ( processingUnitSeq, period ,periodSeq , tenantId, v_Interfaz_Proceso );

                -- Actualizamos la fecha del informes en la tabla 
                p_Actualiza_Informe_Fecha ( period, 'PREFACT_IS');   
            end if;
        end if;
