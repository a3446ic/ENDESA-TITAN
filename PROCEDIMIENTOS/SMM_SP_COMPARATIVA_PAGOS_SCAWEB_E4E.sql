CREATE OR REPLACE PROCEDURE EXT.SMM_SP_COMPARATIVA_PAGOS_SCAWEB_E4E( IN i_processingUnitSeq BIGINT ,IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	--SMM_COMP_SCAWEB_E4E
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_COMP_SCAWEB_E4E.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_COMP_SCAWEB_E4E WHERE PERIODO = i_period;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_COMP_SCAWEB_E4E.', v_log_count, v_idproceso,'info');
	

    
    CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Insertando CREDITOS de datos en tabla EXT.SMM_COMP_SCAWEB_E4E.' , v_log_count, v_idproceso,'info');
    
	INSERT INTO EXT.SMM_COMP_SCAWEB_E4E ( PERIODO, IDPROVEEDOR, DESCRIPCION, ACTIVIDAD, PDS, NOMBRE_FISCAL, IMPORTE_SCAWEB, E4E_POS_CON_CONTRATO, 
...
/* 01.08.2023 DCR v2.15 Quitamos truncados y redondearemos en informes */
    ...
    FROM
        ( select PERIODO, TRIM(to_char(PROVEEDOR,'000')) IDPROVEEDOR, SCA.CODIGO_AGENTE_INTERNO as PDS, sum(REALVALUE) as IMPORTE_SCAWEB, count(*) registros
            from EXT.SMM_SCAWEB_LIQUIDACION sca where PERIODO = i_period
        ...
        /*  
        LEFT JOIN EXT.SMM_E4E_NEGATIVOS_TEMP E4ENT
		...
		*/
		LEFT JOIN 
			...
			from EXT.SMM_E4E_NEGATIVOS_TEMP
			where periodseq=i_periodseq
			group by TRIM(IDPROVEEDOR), PDS
			)E4ENT
			ON TRIM(T_SCAWEB.PDS) = TRIM(E4ENT.PDS)
    ...
    CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin Carga Registros SCAWEB-E4E de la tabla SMM_COMP_SCAWEB_E4E: '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso,'info');

    CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Insertando Registros E4E-SCAWEB (scaweb nulos) de datos en tabla SMM_COMP_SCAWEB_E4E.' , v_log_count, v_idproceso,'info');
    ...
    FROM
        ( SELECT i_period PERIODO, TRIM(IDPROVEEDOR)  AS IDPROVEEDOR, PDS, 
            SUM(CASE WHEN VALUE > 0 AND COD_CONTRATO IS NOT NULL THEN VALUE ELSE 0 END) AS POSITIVO_CON_CONTRATO,
            SUM(CASE WHEN VALUE > 0 AND COD_CONTRATO IS NULL THEN VALUE ELSE 0 END) AS POSITIVO_SIN_CONTRATO,
            SUM(CASE WHEN VALUE < 0 THEN VALUE ELSE 0 END) AS NEGATIVO,
            SUM(VALOR_OPERACIONES) AS OPERACIONES,
            ...
            FROM EXT.SMM_E4E_DEPOSIT_TEMP 
            WHERE PERIODSEQ = i_periodseq
            GROUP BY i_period, TRIM(IDPROVEEDOR), PDS 
        ) T_E4E
        ...
        LEFT JOIN
            ( SELECT 
                ...
                COUNT(*) REGISTROS
                FROM EXT.SMM_SCAWEB_LIQUIDACION SCA
    ...
    CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin Carga INCENTIVOS de la tabla SMM_COMP_SCAWEB_E4E: '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso,'info');
    
   
        
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla SMM_COMP_SCAWEB_E4E '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
