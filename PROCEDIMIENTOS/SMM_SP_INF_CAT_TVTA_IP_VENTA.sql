CREATE PROCEDURE EXT.SMM_SP_INF_CAT_TVTA_IP_VENTA( IN i_period VARCHAR(25), IN i_interfaz NVARCHAR(50))
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	--SMM_CAT_TVTA_IP_VENTA
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_CAT_TVTA_IP_VENTA.', v_log_count, v_idproceso,'info');
	DELETE FROM EXT.SMM_CAT_TVTA_IP_VENTA WHERE PERIODO = i_period;
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_CAT_TVTA_IP_VENTA.', v_log_count, v_idproceso,'info');
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Insertando Registros de datos en tabla EXT.SMM_CAT_TVTA_IP_VENTA.' ,v_log_count, v_idproceso,'info');
    
	INSERT INTO EXT.SMM_CAT_TVTA_IP_VENTA ( PERIODSEQ, PAYEESEQ, CODIGO_COMERCIAL, POSITIONSEQ, PERIODO, ACTIVIDAD, TIPO_PENALIZACION, NUMERO_VENTAS, IMPORTE_PENALIZACION, TOTAL_PENALIZACION ) 
    SELECT 
		PERIODSEQ,
		PAYEESEQ,
		CODIGO_COMERCIAL,
	...
	FROM EXT.SMM_CAT_TVTA_IP_VENTA_TEMP
	WHERE
		PERIODO =  i_period
	;
	...
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_CAT_TVTA_IP_VENTA '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso, 'info');
