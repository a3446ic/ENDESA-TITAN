CREATE OR REPLACE PROCEDURE EXT.SMM_SP_TEMPORAL_CONTRATOS_E4E( IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	--SMM_LIQSCAWEB_FINAL
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio borrado de la tabla SMM_E4E_CONTRATOS_TEMP.', v_log_count, v_idproceso,'info');
	EXECUTE IMMEDIATE 'TRUNCATE TABLE EXT.SMM_E4E_CONTRATOS_TEMP';
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Fin borrado de la tabla SMM_E4E_CONTRATOS_TEMP.', v_log_count, v_idproceso,'info');
	
	INSERT INTO EXT.SMM_E4E_CONTRATOS_TEMP(TENANTID,PERIODSEQ,ID,PDS,ACTIVIDAD_DETALLADA,ACTIVIDAD,
