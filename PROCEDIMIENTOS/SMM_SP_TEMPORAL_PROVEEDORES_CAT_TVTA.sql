CREATE OR REPLACE PROCEDURE EXT.SMM_SP_TEMPORAL_PROVEEDORES_CAT_TVTA( IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Inicio Truncado de la tabla SMM_PROVEEDORES_TEMP_CAT_TVTA.', v_log_count, v_idproceso,'info');
	EXECUTE IMMEDIATE 'TRUNCATE TABLE EXT.SMM_PROVEEDORES_TEMP_CAT_TVTA';
	
	CALL EXT.LIB_GLOBAL_ENDESA:WRITE_DEBUG (v_tenantid,v_permisos_log, v_proc_name,'Cargando tabla SMM_PROVEEDORES_TEMP_CAT_TVTA. Periodo: ' || i_period, v_log_count, v_idproceso,'info');
	INSERT INTO EXT.SMM_PROVEEDORES_TEMP_CAT_TVTA( TENANTID,PERIODSEQ,IDPROVEEDOR,DESCRIPCION,DESCRIPCION_CORTA, FICHERO,CECO,                                                                                                                                                                                                      
 ...
 WHERE                                                                                                                                                                                                                                                                                                                                
  GCT.NAME  like 'Proveedor%';  
    ...
    -- filas := sql%rowcount;
    COMMIT;
    CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga Registros Créditos de la tabla SMM_PROVEEDORES_TEMP_CAT_TVTA: '|| ::ROWCOUNT || ' filas.', v_log_count, v_idproceso,'info');
