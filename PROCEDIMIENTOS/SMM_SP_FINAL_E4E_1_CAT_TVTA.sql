CREATE OR REPLACE PROCEDURE EXT.SMM_SP_FINAL_E4E_1_CAT_TVTA( IN i_period VARCHAR(25), IN i_periodseq BIGINT)
LANGUAGE SQLSCRIPT
...
BEGIN
	...
	DELETE FROM EXT.SMM_E4E_FINAL_CAT_TVTA WHERE PERIODO = i_period AND FICHERO LIKE 'CAT_TVTA1';
	...
	*/
    ...
    if contadorE4E > 4 THEN
		CALL EXT.SMM_SP_CABECERA_FICHEROS_E4E(  i_period , 'CAT_TVTA1' );
	end if;
    
        
	CALL LIB_GLOBAL_ENDESA:WRITE_DEBUG(v_tenantid,v_permisos_log, v_proc_name,'Fin Carga de la tabla EXT.SMM_E4E_FINAL_CAT_TVTA:  E4E1'|| to_char(contadorE4E) || ' -- ECS1'|| to_char(contadorECS) || ' filas.', v_log_count, v_idproceso, 'info');
